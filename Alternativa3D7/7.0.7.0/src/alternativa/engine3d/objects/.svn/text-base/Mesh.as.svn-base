package alternativa.engine3d.objects {
	...
	public class Mesh extends Object3D {
		...
		private function backfaceCull():void {
			var indicesLength:int = poly ? indices.length : numFaces*3;
			
			var dynamicNormals:Boolean = backfaceCulling == 0 || sorting == 2;
			...
			for (var i:int = 0, k:int = 0; i < indicesLength; i = k) {
				if (poly) {
					num = indices[i]; i++;
					k++;
				}
				...
				if (backfaceCulling == 1) {
					if (normals[ni++]*cameraX + normals[ni++]*cameraY + normals[ni++]*cameraZ <= normals[ni++]) continue;
					if (sorting == 2) {
						
					}
				} else {
					if (nx*anx + ny*any + nz*anz >= 0) continue;
				}
