package alternativa.engine3d.core {
	...
	public class Geometry {
		...
		alternativa3d var next:Geometry;
	
		alternativa3d var faceStruct:Face;
	
		private var transformID:int = 0;
		...
		alternativa3d var boundType:int = 0;
		...
		static alternativa3d function create():Geometry {
			if (collector != null) {
		...
		alternativa3d function create():Geometry {
			if (collector != null) {
				var res:Geometry = collector;
				collector = collector.next;
				res.next = null;
		...
		alternativa3d function calculateOOBB():void {
			if (viewAligned) {
		...
		alternativa3d function updateBB(struct:Face):void {
			for (var face:Face = struct; face != null; face = face.processNext) {
				for (var wrapper:Wrapper = face.wrapper; wrapper != null; wrapper = wrapper.next) {
					var vertex:Vertex = wrapper.vertex;
					if (vertex.transformID != transformID) {
		...
		alternativa3d function draw(camera:Camera3D, object:Object3D, parentCanvas:Canvas, threshold:Number):void {
			var canvas:Canvas;
			var list:Face;
			if (viewAligned) {
				list = faceStruct;
