	...
	public class TextureMaterial extends Material {
		...
		override alternativa3d function draw(camera:Camera3D, canvas:Canvas, list:Face, distance:Number):void {
			...
			if (mipMapping < 2) {
				...
				if (mipMapping == 0) {
					// Без мипмаппинга
					canvas.gfx.beginBitmapFill(texture, null, repeat, smooth);
				} else {
					...
					canvas.gfx.beginBitmapFill(mipMap[level], null, repeat, smooth);
				}
				...
				for (var i:int = maxLevel; i >= minLevel; i--) {
					...
					indices.length = indicesLength;
					// Отрисовка
					canvas.gfx.beginBitmapFill(mipMap[i], null, repeat, smooth);
					canvas.gfx.drawTriangles(vertices, indices, uvts, "none");
				}
			}
