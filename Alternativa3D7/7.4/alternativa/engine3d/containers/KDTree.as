package alternativa.engine3d.containers {
	...
	public class KDTree extends ConflictContainer {
		...
		private function drawNode(node:KDNode, culling:int, camera:Camera3D, object:Object3D, canvas:Canvas, geometry:Geometry):void {
			...
			var nodeObjectsBounds:Vector.<Object3D> = node.objectsBounds;
			var nodeObjectsLength:int = node.objectsLength;
			var nodeOccluders:Vector.<Object3D> = node.occluders;
			var nodeOccludersBounds:Vector.<Object3D> = node.occludersBounds;
		...
		public function createTree(staticObjects:Vector.<Object3D>, staticOccluders:Vector.<Object3D> = null):void {
			...
			var objectsBounds:Vector.<Object3D>;
			var objectsLength:int = 0;
			var occluders:Vector.<Object3D>;
			var occludersBounds:Vector.<Object3D>;
			...
			for (i = 0; i < staticObjectsLength; i++) {
				...
				if (bound.boundMinX <= bound.boundMaxX) {
					if (objects == null) {
						...
						objectsBounds = new Vector.<Object3D>();
					}
					objects[objectsLength] = object;
					objectsBounds[objectsLength] = bound;
					objectsLength++;
			...
			for (i = 0; i < staticOccludersLength; i++) {
				...
				if (bound.boundMinX <= bound.boundMaxX) {
					...
					if (bound.boundMinX < minX || bound.boundMaxX > maxX || bound.boundMinY < minY || bound.boundMaxY > maxY || bound.boundMinZ < minZ || bound.boundMaxZ > maxZ) {
						...
						if (occluders == null) {
							occluders = new Vector.<Object3D>();
							occludersBounds = new Vector.<Object3D>();
						}
						occluders[occludersLength] = object;
						occludersBounds[occludersLength] = bound;
						occludersLength++;
					}
				}
			}
			...
			if (objectsLength > 0) {
				...
				root.boundMaxY = maxY;
				root.boundMaxZ = maxZ;
				createNode(root, objects, objectsBounds, objectsLength, occluders, occludersBounds, occludersLength);
			} else {
				root = null;
			}
		}
		...
		private function createNode(node:KDNode, objects:Vector.<Object3D>, objectsBounds:Vector.<Object3D>, numObjects:int, occluders:Vector.<Object3D>, occludersBounds:Vector.<Object3D>, numOccluders:int):void {
			var i:int;
			var j:int;
			var bound:Object3D;
			var coord:Number;
			...
			for (i = 0; i < numObjects; i++) {
				bound = objectsBounds[i];
				if (bound.boundMaxX - bound.boundMinX <= threshold + threshold) {
					coord = (bound.boundMinX <= node.boundMinX + threshold) ? node.boundMinX : ((bound.boundMaxX >= node.boundMaxX - threshold) ? node.boundMaxX : (bound.boundMinX + bound.boundMaxX)*0.5);
					...
					if (bound.boundMinX > node.boundMinX + threshold) {
					...
					if (bound.boundMaxX < node.boundMaxX - threshold) {
				...
				if (bound.boundMaxY - bound.boundMinY <= threshold + threshold) {
					coord = (bound.boundMinY <= node.boundMinY + threshold) ? node.boundMinY : ((bound.boundMaxY >= node.boundMaxY - threshold) ? node.boundMaxY : (bound.boundMinY + bound.boundMaxY)*0.5);
					...
					if (bound.boundMinY > node.boundMinY + threshold) {
					...
					if (bound.boundMaxY < node.boundMaxY - threshold) {
				...
				if (bound.boundMaxZ - bound.boundMinZ <= threshold + threshold) {
					coord = (bound.boundMinZ <= node.boundMinZ + threshold) ? node.boundMinZ : ((bound.boundMaxZ >= node.boundMaxZ - threshold) ? node.boundMaxZ : (bound.boundMinZ + bound.boundMaxZ)*0.5);
					...
					if (bound.boundMinZ > node.boundMinZ + threshold) {
					...
					if (bound.boundMaxZ < node.boundMaxZ - threshold) {
			...
			if (splitAxis < 0) {
				...
				node.objectsBounds = objectsBounds;
				node.objectsLength = numObjects;
				node.occluders = occluders;
				node.occludersBounds = occludersBounds;
					...
					if (max <= splitCoord + threshold) {
						if (min < splitCoord - threshold) {
							...
							if (negativeObjects == null) {
								negativeObjects = new Vector.<Object3D>();
								negativeObjectsBounds = new Vector.<Object3D>();
							}
							negativeObjects[negativeObjectsLength] = objects[i];
							negativeObjectsBounds[negativeObjectsLength] = bound;
							negativeObjectsLength++;
						} else {
							// Остаётся в ноде
							if (node.objects == null) {
								node.objects = new Vector.<Object3D>();
								node.objectsBounds = new Vector.<Object3D>();
							}
							node.objects[node.objectsLength] = objects[i];
							node.objectsBounds[node.objectsLength] = bound;
							node.objectsLength++;
						}
					} else {
						if (min >= splitCoord - threshold) {
							...
							if (positiveObjects == null) {
								positiveObjects = new Vector.<Object3D>();
								positiveObjectsBounds = new Vector.<Object3D>();
							}
							positiveObjects[positiveObjectsLength] = objects[i];
							positiveObjectsBounds[positiveObjectsLength] = bound;
							positiveObjectsLength++;
						} else {
							// Распилился
						}
					}
				...
				for (i = 0; i < numOccluders; i++) {
					bound = occludersBounds[i];
					min = (splitAxis == 0) ? bound.boundMinX : ((splitAxis == 1) ? bound.boundMinY : bound.boundMinZ);
					max = (splitAxis == 0) ? bound.boundMaxX : ((splitAxis == 1) ? bound.boundMaxY : bound.boundMaxZ);
					if (max <= splitCoord + threshold) {
						if (min < splitCoord - threshold) {
							// Объект в негативной стороне
							if (negativeOccluders == null) {
								negativeOccluders = new Vector.<Object3D>();
								negativeOccludersBounds = new Vector.<Object3D>();
							}
							negativeOccluders[negativeOccludersLength] = occluders[i];
							negativeOccludersBounds[negativeOccludersLength] = bound;
							negativeOccludersLength++;
						} else {
							// Остаётся в ноде
							if (node.occluders == null) {
								node.occluders = new Vector.<Object3D>();
								node.occludersBounds = new Vector.<Object3D>();
							}
							node.occluders[node.occludersLength] = occluders[i];
							node.occludersBounds[node.occludersLength] = bound;
							node.occludersLength++;
						}
					} else {
						if (min >= splitCoord - threshold) {
							// Объект в положительной стороне
							if (positiveOccluders == null) {
								positiveOccluders = new Vector.<Object3D>();
								positiveOccludersBounds = new Vector.<Object3D>();
							}
							positiveOccluders[positiveOccludersLength] = occluders[i];
							positiveOccludersBounds[positiveOccludersLength] = bound;
							positiveOccludersLength++;
						} else {
							// Распилился
							trace("Incorrect occluder size or position");
						}
				...
				if (negativeObjectsLength > 0) {
					createNode(node.negative, negativeObjects, negativeObjectsBounds, negativeObjectsLength, negativeOccluders, negativeOccludersBounds, negativeOccludersLength);
				} else if (negativeOccludersLength > 0) {
					trace("Incorrect occluder size or position");
				}
				if (positiveObjectsLength > 0) {
					createNode(node.positive, positiveObjects, positiveObjectsBounds, positiveObjectsLength, positiveOccluders, positiveOccludersBounds, positiveOccludersLength);
