package alternativa.engine3d.objects {
	...
	public class Mesh extends Object3D {
		...
		alternativa3d var faceTree:Face;
		alternativa3d var bspVertexList:Vertex;
		...
		public function removeVertex(vertex:Vertex):Vertex {
			...
			var prev:Vertex;
			var curr:Vertex = vertexList;
			while (curr != null) {
				if (curr == vertex) break;
				prev = curr;
				curr = curr.next;
			}
			if (curr == null) throw new ArgumentError("Vertex not found.");
			var vertexFaces:Vector.<Face> = getVertexFaces(vertex);
			var len:int = vertexFaces.length;
			for (var i:int = 0; i < len; i++) {
				removeFace(vertexFaces[i]);
			}
			if (prev != null) {
				prev.next = vertex.next;
			} else {
				vertexList = vertex.next;
			}
			vertex.next = null;
			return vertex;
		}
		...
		public function addFace(vertices:Vector.<Vertex>, material:Material = null):Face {
			...
			for (var i:int = 0; i < verticesLength; i++) {
				...
				if (v == null) throw new ArgumentError("Vertices must be in mesh.");
			...
			newFace.calculateNormal(true);
			return newFace;
		}
		
		public function addTriFace(v1:Vertex, v2:Vertex, v3:Vertex, material:Material = null):Face {
			if (v1 == null) throw new TypeError("Parameter v1 must be non-null.");
			if (v2 == null) throw new TypeError("Parameter v2 must be non-null.");
			if (v3 == null) throw new TypeError("Parameter v3 must be non-null.");
			var v:Vertex = vertexList;
			while (v != null) {
				if (v == v1) break;
				v = v.next;
			...
			newFace.wrapper.vertex = v1;
			newFace.wrapper.next = new Wrapper();
			newFace.wrapper.next.vertex = v2;
			newFace.wrapper.next.next = new Wrapper();
			newFace.wrapper.next.next.vertex = v3;
			if (faceList != null) {
				var face:Face = faceList;
				while (face.next != null) face = face.next;
				face.next = newFace;
		...
		public function addQuadFace(v1:Vertex, v2:Vertex, v3:Vertex, v4:Vertex, material:Material = null):Face {
			...
			if (v2 == null) throw new TypeError("Parameter v2 must be non-null.");
			if (v3 == null) throw new TypeError("Parameter v3 must be non-null.");
			if (v4 == null) throw new TypeError("Parameter v4 must be non-null.");
			var v:Vertex = vertexList;
			while (v != null) {
				if (v == v1) break;
				v = v.next;
			...
			newFace.wrapper.next.next.next.vertex = v4;
			if (faceList != null) {
				var face:Face = faceList;
				while (face.next != null) face = face.next;
				face.next = newFace;
		...
		public function removeFace(face:Face):Face {
			...
			var prev:Face;
			var curr:Face = faceList;
			while (curr != null) {
				if (curr == face) break;
				prev = curr;
				curr = curr.next;
			}
			if (curr == null) throw new ArgumentError("Face not found.");
			if (prev != null) {
				prev.next = face.next;
			} else {
				faceList = face.next;
		...
		public function addGeometryByIndices(vertices:Vector.<Number>, uvs:Vector.<Number>, indices:Vector.<int>, poly:Boolean = false, material:Material = null):void {
			if (vertices == null) throw new TypeError("Parameter vertices must be non-null.");
			var i:int, j:int, k:int;
			var length:int = vertices.length/3;
			if (uvs != null && length != uvs.length/2) throw new ArgumentError("Vertices count and uvs count doesn't match.");
			var verts:Vector.<Vertex> = new Vector.<Vertex>(length);
			var lastVertex:Vertex;
			if (vertexList != null) {
				lastVertex = vertexList;
				while (lastVertex.next != null) lastVertex = lastVertex.next;
			}
			for (i = 0, j = 0, k = 0; i < length; i++) {
				var newVertex:Vertex = new Vertex();
				newVertex.x = vertices[j];
				j++;
				newVertex.y = vertices[j];
				j++;
				newVertex.z = vertices[j];
				j++;
				if (uvs != null) {
					newVertex.u = uvs[k];
					k++;
			...
			if (indices != null) {
				...
				for (i = 0, k = 0; i < indicesLength; i++) {
					...
					if (index < 0 || index >= length) throw new RangeError("Index is out of bounds.");
				}
				var lastFace:Face;
				if (faceList != null) {
					lastFace = faceList;
					while (lastFace.next != null) lastFace = lastFace.next;
				}
				var lastWrapper:Wrapper;
				for (i = 0, k = 0; i < indicesLength;) {
					if (i == k) {
						k = poly ? (indices[i] + ++i) : (i + 3);
						if (lastFace != null) {
							lastFace.next = new Face();
					...
					if (lastWrapper != null) {
						lastWrapper.next = new Wrapper();
						lastWrapper = lastWrapper.next;
					} else {
						lastFace.wrapper = new Wrapper();
						lastWrapper = lastFace.wrapper;
					}
					lastWrapper.vertex = verts[indices[i]];
					if (++i == k) {
						lastFace.material = material;
						lastFace.calculateNormal(true);
					}
				}
			}
		}
		
		public function getVertexFaces(vertex:Vertex):Vector.<Face> {
			if (vertex == null) throw new TypeError("Parameter vertex must be non-null.");
			var v:Vertex = vertexList;
			while (v != null) {
				if (v == vertex) break;
			...
			if (v == null) throw new ArgumentError("Vertex not found.");
		...
		override alternativa3d function draw(camera:Camera3D, object:Object3D, parentCanvas:Canvas):void {
			...
			if (transformID > 500000000) {
				transformID = 0;
				for (vertex = vertexList; vertex != null; vertex = vertex.next) vertex.transformID = 0;
				for (vertex = bspVertexList; vertex != null; vertex = vertex.next) vertex.transformID = 0;
			}
			// Коррекция куллинга
		...
		public function weldVertices(distanceThreshold:Number = 0, uvThreshold:Number = 0):void {
			...
			// Группировка
			group(vertices, 0, verticesLength, 0, distanceThreshold, uvThreshold, new Vector.<int>());
			// Замена вершин
			for (face = faceList; face != null; face = face.next) {
				for (wrapper = face.wrapper; wrapper != null; wrapper = wrapper.next) {
					vertex = wrapper.vertex;
		...
		private function group(vertices:Vector.<Vertex>, begin:int, end:int, depth:int, distanceThreshold:Number, uvThreshold:Number, stack:Vector.<int>):void {
			var i:int;
			var j:int;
			var vertex:Vertex;
			...
			for (j = i + 1; j < end; j++) {
				...
				if (vertex.offset - compared.offset > threshold) {
					if (depth < 4 && j - i > 1) {
						group(vertices, i, j, depth + 1, distanceThreshold, uvThreshold, stack);
					}
					i = j;
					vertex = vertices[i];
				} else if (depth == 4) {
