package alternativa.engine3d.objects {
	...
	public class Mesh extends Object3D {
		 ...
		 * 0 - отсечение по динамически расчитываемым временным нормалям
		...
		static private const dynamicNormals:Vector.<Number> = new Vector.<Number>();
		...
		override alternativa3d function draw(camera:Camera3D, object:Object3D, parentCanvas:Canvas):void {
			...
			if (backfaceCulling == 1 || sorting == 3) calculateInverseCameraMatrix(object.cameraMatrix);
				...
				// Динамическое BSP
					...
					if (resultIndicesLength > 0) {
						...
						if (fragmentsLength > 0) {
							resultIndicesLength = 0;
							collectDynamicNode(0, fragmentsLength, numVertices);
						}
					}
				// Статическое BSP
				} else {
			...
			// Клиппинг
				...
				// Динамическое BSP
					...
					if (resultIndicesLength > 0) {
						...
						if (fragmentsLength > 0) {
							resultIndicesLength = 0;
							collectDynamicNode(0, fragmentsLength, cameraVertices.length/3);
						}
					}
				// Статическое BSP
				} else {
				...
				if (clipping != 0 || !(object.culling & 1)) {
					...
					if (debugResult & Debug.VERTICES) {
						...
						for (i = 0, k = 0; i < resultIndicesLength; i++) {
							if (i == k) k = (poly || sorting == 3) ? (resultIndices[i++] + i) : (i + 3);
					...
					if (debugResult & Debug.NORMALS) {
						...
						for (i = 0, k = 0, n = 0; i < resultIndicesLength; i = k) {
							k = (poly || sorting == 3) ? (resultIndices[i++] + i) : (i + 3);
							x = y = z = 0;
							for (j = i; j < k; j++) {
		...
		private function getFragments(geometry:Geometry, source:Vector.<int>, sourceLength:int, poly:Boolean, culling:int, near:Number, far:Number):Fragment {
			...
			for (var i:int = 0; i < sourceLength; i = k) {
				...
				if (sorting != 3) {
					...
					if (sorting == 2 || backfaceCulling == 0) {
						a = i;
						vi = source[a]*3; a++;
			...
			if (backfaceCulling == 1) {
				...
				for (var ni:int = 0; i < sourceIndicesLength;) {
					if (normals[ni++]*cameraX + normals[ni++]*cameraY + normals[ni++]*cameraZ > normals[ni++]) {
						resultIndices[resultIndicesLength++] = sourceIndices[i++], resultIndices[resultIndicesLength++] = sourceIndices[i++], resultIndices[resultIndicesLength++] = sourceIndices[i++];
					}
					else i += 3;
				}
			} else {
		...
		public function calculateNormals(normalize:Boolean = false):void {
			...
			// Расчитываем нормали
			for (var i:int = 0, j:int = 0, num:int = 3, vi:int, indicesLength:int = indices.length; i < indicesLength; i += num) {
		...
		public function calculateBSP(splitAnalysis:Boolean = false):void {
			...
			// Построение дерева
			bsp = createNode(0, fragmentsLength, splitAnalysis);
		}
		
		private function prepareToDynamicBSP():void {
			fragmentsLength = 0;
			for (var i:int = 0, num:int = 3, ni:int = 0; i < resultIndicesLength;) {
				if (poly) num = resultIndices[i++];
				fragments[fragmentsLength++] = (ni << 16) + num;
				var vi:int;
				var ax:Number = cameraVertices[vi = int(resultIndices[i]*3)];
				var ay:Number = cameraVertices[++vi];
				var az:Number = cameraVertices[++vi];
				...
				ny *= nl;
				nz *= nl;
				dynamicNormals[ni++] = nx; 
				dynamicNormals[ni++] = ny; 
				dynamicNormals[ni++] = nz; 
				dynamicNormals[ni++] = ax*nx + ay*ny + az*nz;
				for (var j:int = i - num; j < i; j++) {
					fragments[fragmentsLength++] = resultIndices[j];
				}
			}
		}
		
		private function collectDynamicNode(begin:int, end:int, v:int):int {
			// Построение ноды
			var mi:int = fragments[begin], ni:int = mi >> 16, num:int = mi & 0xFFFF;
			var normalX:Number = dynamicNormals[ni++];
			var normalY:Number = dynamicNormals[ni++];
			var normalZ:Number = dynamicNormals[ni++];
			var offset:Number = dynamicNormals[ni];
			...
			var nodeBegin:int = fragmentsLength + reserve;
			var nodeEnd:int = nodeBegin;
			var positiveBegin:int = fragmentsLength + reserve + reserve;
			...
			var i:int;
			var k:int = begin + num + 1;
			// Добавление сплиттера
			fragments[nodeEnd++] = num;
			for (i = begin + 1; i < k; i++) {
				fragments[nodeEnd++] = fragments[i];
			}
			// Перебираем грани
			for (var j1:int = negativeEnd, j2:int = positiveEnd, vi:int = v*3, infront:Boolean, behind:Boolean, t:Number, uv:Number; i < end;) {
				if (i == k) {
					...
					var ax:Number = cameraVertices[ai], ay:Number = cameraVertices[int(ai + 1)], az:Number = cameraVertices[int(ai + 2)];
					var ao:Number = ax*normalX + ay*normalY + az*normalZ - offset;
				}
				// Вторая точка ребра
				var b:int = fragments[i], bi:int = b*3;
				var bx:Number = cameraVertices[bi], by:Number = cameraVertices[int(bi + 1)], bz:Number = cameraVertices[int(bi + 2)];
				var bo:Number = bx*normalX + by*normalY + bz*normalZ - offset;
				// Рассечение ребра
				if (ao < -threshold && bo > threshold || bo < -threshold && ao > threshold) t = ao/(ao - bo), cameraVertices[vi] = ax + (bx - ax)*t, uvts[vi++] = (uv = uvts[ai]) + (uvts[bi] - uv)*t, cameraVertices[vi] = ay + (by - ay)*t, uvts[vi++] = (uv = uvts[int(ai + 1)]) + (uvts[int(bi + 1)] - uv)*t, cameraVertices[vi] = az + (bz - az)*t, uvts[vi++] = 0, fragments[j1++] = fragments[j2++] = v++;
				// Добавление точки
				if (bo < -threshold) {
				...
				if (++i == k) {
					if (infront && behind) {
						...
						fragments[nodeEnd++] = num;
						for (var j:int = k - num; j < k; j++) {
							fragments[nodeEnd++] = fragments[j];
						}
			...
			if (negativeEnd > negativeBegin) {
				v = collectDynamicNode(negativeBegin, negativeEnd, v);
			}
			// Сбор фрагментов в плоскости ноды
			for (i = nodeBegin, k = nodeBegin; i < nodeEnd;) {
				if (i == k) k = fragments[i++] + i, a = fragments[i++], b = fragments[i++];
				resultIndices[resultIndicesLength++] = a, resultIndices[resultIndicesLength++] = b, resultIndices[resultIndicesLength++] = b = fragments[i++];
			}
			// Разделение передней части
			if (positiveEnd > positiveBegin) {
				v = collectDynamicNode(positiveBegin, positiveEnd, v);
			}
			return v;
		}
		
		private function createNode(begin:int, end:int, splitAnalysis:Boolean):BSPNode {
			var i:int;
			var j:int;
			var k:int;
			var num:int;
			...
			var splitter:int = begin;
			if (splitAnalysis) {
				var bestSplits:int = int.MAX_VALUE;
				// Перебираем нормали
				for (i = begin; i < end; i += (mi & 0xFFFF) + 1) {
			...
			if (fragmentsLength > fragmentsRealLength) {
				fragments.length = fragmentsLength;
				fragmentsRealLength = fragmentsLength;
			}
			// Перебираем грани
			...
			for (i = begin, k = begin, vi = numVertices*3; i < end;) {
				...
				if (++i == k) {
					if (infront && behind) {
						// Фрагмент распилился
						fragments[negativeEnd] = (ni << 16) + j1 - negativeEnd - 1;
						negativeEnd = j1;
						fragments[positiveEnd] = (ni << 16) + j2 - positiveEnd - 1;
						positiveEnd = j2;
					} else if (infront) {
						// Фрагмент спереди
						fragments[positiveEnd] = (ni << 16) + j2 - positiveEnd - 1;
						...
						// Фрагмент в плоскости ноды
						normalX = normals[ni]; ii = ni + 1;
						normalY = normals[ii]; ii++;
						normalZ = normals[ii];
						if (node.normalX*normalX + node.normalY*normalY + node.normalZ*normalZ > 0) {
							// Фрагмент сонаправлен ноде
							node.addFragment(fragments, k - num, k);
							j1 = negativeEnd;
							j2 = positiveEnd;
						} else {
							// Фрагмент противонаправлен ноде
							fragments[negativeEnd] = (ni << 16) + j1 - negativeEnd - 1;
							negativeEnd = j1;
							j2 = positiveEnd;
						}
			...
			// Разделение заднй части
			if (negativeEnd > negativeBegin) {
				node.negative = createNode(negativeBegin, negativeEnd, splitAnalysis);
			}
			// Разделение передней части
			if (positiveEnd > positiveBegin) {
				node.positive = createNode(positiveBegin, positiveEnd, splitAnalysis);
			}
			return node;
		}
		...
		public function weldVertices(distanceThreshold:Number = 0, uvThreshold:Number = 0):void {
			...
			uselessIndices.sort(function(x:int, y:int):Number {return x - y;});
	...
	class BSPNode {
		...
		public var polygonsLength:int = 0;
