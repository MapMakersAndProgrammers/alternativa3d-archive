package alternativa.engine3d.containers {
	...
	public class KDTree extends ConflictContainer {
		...
		public function optimizeTree(disposeSource:Boolean = true):Vector.<TextureMaterial> {
			var meshes:Dictionary = new Dictionary();
			var materials:Vector.<TextureMaterial> = new Vector.<TextureMaterial>();
			//optimizeNode(root, materials, meshes);
			optimizeNode2(root, meshes);
			...
			if (disposeSource) {
				for (var key:* in meshes) {
					...
					material.disposeMipMap();
				}
			}
			return materials;
		}
		
		private function optimizeNode2(node:KDNode, src:Dictionary):void {
			if (node != null) {
				
				var newObjects:Vector.<Object3D> = new Vector.<Object3D>();
				
				for (var i:int = 0; i < node.objectsLength; i++) {
					var object:Object3D = node.objects[i];
					object.composeMatrix();
					...
					var newMesh:Mesh = mesh.clone() as Mesh;
					newMesh.clipping = Clipping.FACE_CLIPPING;
					newMesh.sorting = Sorting.AVERAGE_Z;
					newMesh.setMatrix(new Matrix3D());
					
					material.disposeMipMap();
					var newMaterial:TextureMaterial = new TextureMaterial(material.texture.clone(), material.repeat, material.smooth);
					newMaterial.calculateMipMaps(3);
					newMaterial.resolution = 5;
					newMaterial.mipMapping = MipMapping.PER_PIXEL;
					
					for (var face:Face = newMesh.faceList; face != null; face = face.next) {
						face.material = newMaterial;
					}
					for (var vertex:Vertex = newMesh.vertexList; vertex != null; vertex = vertex.next) {
						var x:Number = vertex.x;
						var y:Number = vertex.y;
						var z:Number = vertex.z;
						vertex.x = object.ma*x + object.mb*y + object.mc*z + object.md;
						vertex.y = object.me*x + object.mf*y + object.mg*z + object.mh;
						vertex.z = object.mi*x + object.mj*y + object.mk*z + object.ml;
					}
					newMesh.calculateBounds();
					newMesh.calculateNormals(true);
					if (mesh.sorting == Sorting.STATIC_BSP) {
						newMesh.calculateBSP(true);
					}
					newMesh.sorting = mesh.sorting;
					newMesh.clipping = mesh.clipping;
					newObjects.push(newMesh);
				}
				
				node.objects = newObjects;
					
				optimizeNode2(node.negative, src);
				optimizeNode2(node.positive, src);
			}
		}
		
		private function optimizeNode(node:KDNode, collector:Vector.<TextureMaterial>, src:Dictionary):void {
			if (node != null) {
				var i:int;
				var vertex:Vertex;
				var face:Face;
				...
				// Узловая нода с двумя или более объектами
				if (node.negative != null && node.objectsLength > 1) {
					
					// Объекты по размеру битмапы
					var sizeMap:Object = new Object();
					for (i = 0; i < node.objectsLength; i++) {
						object = node.objects[i];
						source = object;
						while (source is Reference) {
							source = (source as Reference).referenceObject;
						}
						texture = ((source as Mesh).faceList.material as TextureMaterial).texture;
						sizeMapKey = texture.width + "_" + texture.height;
						objects = sizeMap[sizeMapKey];
						if (objects == null) {
							objects = new Vector.<Object3D>();
							sizeMap[sizeMapKey] = objects;
						}
						objects.push(object);
					}
					
					newMesh = new Mesh();
					newMesh.sorting = Sorting.NONE;
					
					for (var key:* in sizeMap) {
						sizeMapKey = key;
						objects = sizeMap[sizeMapKey];
						var objectsLength:int = objects.length;
						var ti:int = sizeMapKey.indexOf("_");
						var tw:int = int(sizeMapKey.substring(0, ti));
						var th:int = int(sizeMapKey.substring(ti + 1));
						rect.width = tw;
						rect.height = th;
						var numX:int = 4096/tw;
						var numY:int = objectsLength/numX;
						if (numY > 0) {
							var mod:int = objectsLength % numX;
							while (mod > 0 && numX - mod > numY) {
								numX--;
								mod = objectsLength % numX;
							}
						} else {
							numX = objectsLength;
						}
						...
						var atlas:BitmapData = new BitmapData(numX*tw, numY*th, false, 0x000000);
						var atlasMaterial:TextureMaterial = new TextureMaterial(atlas);
						collector.push(atlasMaterial);
						for (i = 0; i < objectsLength; i++) {
							...
							mesh = source as Mesh;
							src[mesh] = true;
							texture = (mesh.faceList.material as TextureMaterial).texture;
							point.x = (i % numX)*tw;
							point.y = int(i/numX)*th;
							atlas.copyPixels(texture, rect, point);
							
							for (face = mesh.faceList; face != null; face = face.next) {
								var newFace:Face = new Face();
								newFace.material = atlasMaterial;
								...
								for (var wrapper:Wrapper = face.wrapper; wrapper != null; wrapper = wrapper.next) {
									...
									newVertex.next = newMesh.vertexList;
									newMesh.vertexList = newVertex;
									newVertex.x = object.ma*vertex.x + object.mb*vertex.y + object.mc*vertex.z + object.md;
									newVertex.y = object.me*vertex.x + object.mf*vertex.y + object.mg*vertex.z + object.mh;
									newVertex.z = object.mi*vertex.x + object.mj*vertex.y + object.mk*vertex.z + object.ml;
									newVertex.u = vertex.u/numX + point.x/(numX*tw);
									newVertex.v = vertex.v/numY + point.y/(numY*th);
									newWrapper.vertex = newVertex;
									if (lastWrapper != null) {
								...
								newFace.next = newMesh.faceList;
								newMesh.faceList = newFace;
							}
							
						}
						atlasMaterial.calculateMipMaps();
						atlasMaterial.mipMapping = MipMapping.PER_PIXEL;
						atlasMaterial.resolution = 5;
						atlasMaterial.correctUV = true;
					}
					
					newMesh.calculateBounds();
					newMesh.calculateNormals(true);
					
					node.objectsLength = 1;
					node.objects.length = 1;
					node.objects[0] = newMesh;
				
				// Листовая нода или узловая с одним или менее объектом
				} else {
					for (i = 0; i < node.objectsLength; i++) {
						object = node.objects[i];
						object.composeMatrix();
						...
						var material:TextureMaterial = mesh.faceList.material as TextureMaterial;
						var newMaterial:TextureMaterial = new TextureMaterial(material.texture.clone(), material.repeat, material.smooth);
						collector.push(newMaterial);
						newMaterial.calculateMipMaps();
						newMaterial.resolution = 5;
						newMaterial.mipMapping = MipMapping.PER_PIXEL;
						newMesh.setMaterialToAllFaces(newMaterial);
						for (vertex = newMesh.vertexList; vertex != null; vertex = vertex.next) {
							var x:Number = vertex.x;
							var y:Number = vertex.y;
							var z:Number = vertex.z;
							vertex.x = object.ma*x + object.mb*y + object.mc*z + object.md;
						...
						newMesh.sorting = mesh.sorting;
						node.objects[i] = newMesh;
					}
				}
				optimizeNode(node.negative, collector, src);
				optimizeNode(node.positive, collector, src);
			}
		}
