package alternativa.engine3d.loaders {
	...
	public class ParserCollada {
		...
		/**
		 * Список родителей объектов. Количество и порядок расположения элементов соответствует массиву <code>objects</code>.
		 * @see #objects
		 */
		...
		/**
		 * Объект -> одна из констант <code>STATIC_GEOMETRY</code>, <code>SPLITTER</code>.
		 * <code>STATIC_OBJECT</code> используется если объекта нет в словаре.
		 */
		...
		public function clean():void {
            ...
            layers = null;
			bspContainerChildrenTypes = null;
		}
		...
		/**
		 * Возвращает функциональное назначение внутри бсп контейнера.
		 * Одно из значений <code>ParserCollada.STATIC_GEOMETRY</code>, <code>ParserCollada.STATIC_OBJECT</code>, <code>ParserCollada.SPLITTER</code>.
		 * @param object Объект.
		 * @return Тип дочернего объекта.
		 */
		public function getBspContainerChildType(object:Object3D):uint {
			return bspContainerChildrenTypes[object];
		}
		...
		private function init(data:XML):DaeDocument {
			...
			bspContainerChildrenTypes = new Dictionary();
		...
		private function addObject(animatedObject:DaeObject, parent:Object3D, layer:String):Object3D {
			...
			if (animatedObject.isStaticGeometry) {
				bspContainerChildrenTypes[object] = STATIC_GEOMETRY;
			} else if (animatedObject.isSplitter) {
				bspContainerChildrenTypes[object] = SPLITTER;
			}
			return object;
		}
		...
		private function parseNodes(nodes:Vector.<DaeNode>, parent:Object3D, skipEmptyObjects:Boolean,  skinsOnly:Boolean = false):void {
			for (var i:int = 0, count:int = nodes.length; i < count; i++) {
				...
				if (node.skins != null) {
					object = addObjects(node.skins, parent, node.layer);
				} else {
					if (!node.skinOrRootJoint && !skinsOnly) {
						if (node.objects != null) {
							object = addObjects(node.objects, parent, node.layer);
						} else {
							if (!skipEmptyObjects) {
								object = new Object3D();
								object.name = node.name;
								object = addObject(node.applyAnimation(node.applyTransformations(object)), parent, node.layer);
							}
						}
					}
				}
				// Если это кость или скин, парсим только скины у детей
				var skinsOnlyChildren:Boolean = skinsOnly || node.skinOrRootJoint;
				if (object == null) {
					if (hasSignifiantChildren(node, skipEmptyObjects,  skinsOnlyChildren)) {
						object = new Object3D();
						object.name = node.name;
						parseNodes(node.nodes, addObject(node.applyAnimation(node.applyTransformations(object)), parent, node.layer), skipEmptyObjects, skinsOnlyChildren);
					}
				} else {
					parseNodes(node.nodes, object, skipEmptyObjects, skinsOnlyChildren);
				}
			}
		}
		...
		private function appendMatrixToObjects(objects:Vector.<DaeObject>, append:Matrix3D):void {
			for (var i:int = 0, count:int = objects.length; i < count; i++) {
				var object:Transform3D = objects[i].object;
					...
					if (!node.skinOrRootJoint && !skinsOnly) {
						if (node.objects != null) {
							...
							if (!skipEmptyObjects) {
								...
								object.name = node.name;
								addObject(node.applyAnimation(node.applyTransformations(object, null, toParentMatrix)), parent, node.layer);
							}
						}
					}
				}
