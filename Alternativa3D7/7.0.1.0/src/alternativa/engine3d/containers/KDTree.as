package alternativa.engine3d.containers {
	...
	public class KDTree extends Object3DContainer {
		 ...
		 */
		public function addWireBounds():void {
			if (rootNode != null) {
				var w:WireBoundBox = new WireBoundBox();
				w.boundBox = rootNode.boundBox;
				addChild(w);
				
				addNodeWireBounds(rootNode, 1);
			}
		}
		
		private function addNodeWireBounds(node:KDNode, alpha:Number):void {
			if (node == null) return;
			...
			addNodeWireBounds(node.negative, alpha);
			addNodeWireBounds(node.positive, alpha);
		}
