package alternativa.engine3d.objects {
	...
	public class KDObject extends Object3D {
		...
		static private const normalsSet:Vector.<Vector.<Number>> = new Vector.<Vector.<Number>>();
		static private var normalsSetRealLength:int = 0;
		/**
		 * @private 
		 */
		static alternativa3d var normalsSetLength:int = 0;
		static private const normalsCalculated:Vector.<int> = new Vector.<int>();
		static private var normalsCalculatedRealLength:int = 0;
		...
		alternativa3d function destroy():void {
			...
			if (bsp != null) bsp.destroy(), bsp = null, normals = null;
		...
		static alternativa3d function createFrom(object:Object3D, camera:Camera3D, matrix:Matrix3D = null):KDObject {
			...
			if (source is Mesh) {
				...
				// Подготовка к ремапу
				for (i = 0; i < mesh.numVertices;) verticesMap[i++] = -1;
				// Создание BSP-дерева
				if (mesh.bsp != null) {
					if (normalsCalculatedRealLength < mesh.numFaces) normalsCalculated.length = normalsCalculatedRealLength = mesh.numFaces;
				...
				if (result.numVertices > 0) {
					...
					// Расчёт нормалей для BSP
					if (result.bsp != null) {
						if (normalsSetLength >= normalsSetRealLength) normalsSet[normalsSetRealLength++] = new Vector.<Number>();
						result.normals = normalsSet[normalsSetLength++];
						if (result.normals.length < mesh.numFaces << 2) result.normals.length = mesh.numFaces << 2;
						for (var i:int = 0, n:int = 0, num:int, vi:int, ni:int, meshIndices:Vector.<int> = mesh.indices, meshVertices:Vector.<Number> = mesh.cameraVertices, indicesLength:int = meshIndices.length; i < indicesLength; i += num, n++) {
							num = meshIndices[i++];
							if (normalsCalculated[n] > 0) {
								normalsCalculated[n] = 0;
								var ax:Number = meshVertices[vi = int(meshIndices[i]*3)], ay:Number = meshVertices[++vi], az:Number = meshVertices[++vi];
								var abx:Number = meshVertices[vi = int(meshIndices[int(i + 1)]*3)] - ax, aby:Number = meshVertices[++vi] - ay, abz:Number = meshVertices[++vi] - az;
								var acx:Number = meshVertices[vi = int(meshIndices[int(i + 2)]*3)] - ax, acy:Number = meshVertices[++vi] - ay, acz:Number = meshVertices[++vi] - az;
								var nx:Number = acz*aby - acy*abz, ny:Number = acx*abz - acz*abx, nz:Number = acy*abx - acx*aby, nl:Number = Math.sqrt(nx*nx + ny*ny + nz*nz); 
								if (nl > 0) nx /= nl, ny /= nl,	nz /= nl;
								result.normals[ni = n << 2] = nx, result.normals[++ni] = ny, result.normals[++ni] = nz,	result.normals[++ni] = ax*nx + ay*ny + az*nz;
							}
						}
					}
		...
		private function clipNode(node:BSPNode, mesh:Mesh, culling:int, near:Number, far:Number):BSPNode {
			if (node != null) {
				...
				var cameraInfront:Boolean = mesh.cameraX*mesh.normals[ni++] + mesh.cameraY*mesh.normals[ni++] + mesh.cameraZ*mesh.normals[ni++] > mesh.normals[ni];
				...
				if (j > 0 || negative != null && positive != null) {
					...
					newNode = node.create();
					normalsCalculated[node.normal] = 1;
					res.normal = node.normal;
		...
		alternativa3d function split(axisX:Boolean, axisY:Boolean, coord:Number, threshold:Number, negative:KDObject, positive:KDObject):void {
			...
			if (bsp != null) {
				...
				if (bsp.negative != null) negative.bsp = bsp.negative, bsp.negative = null, negative.normals = normals;
				if (bsp.positive != null) positive.bsp = bsp.positive, bsp.positive = null, positive.normals = normals;
			// Разбиение полигонов
			} else {
		...
		private function splitNode(node:BSPNode, axisX:Boolean, axisY:Boolean, coord:Number, threshold:Number, negative:KDObject, positive:KDObject):void {
			...
			if (j1 > 0 || negativeNegative != null && positiveNegative != null) {
				...
				node.negative.normal = node.normal;
			...
			if (j2 > 0 || negativePositive != null && positivePositive != null) {
				...
				node.positive.normal = node.normal;
		...
		static alternativa3d function drawConflict(camera:Camera3D, object:Object3D, canvas:Canvas, kdObjects:Vector.<KDObject>, begin:int, end:int, threshold:Number):void {
			...
			// Отрисовка баунда конфликтной зоны
			/*if (Debug.bounds) {
				var boundBox:BoundBox = (kdObjects[begin] as KDObject)._boundBox;
				for (i = begin + 1; i < end; i++) {
					boundBox.addBoundBox((kdObjects[i] as KDObject)._boundBox);
				}
				var containerBoundBox:BoundBox = object._boundBox;
				object._boundBox = boundBox;
				object.drawBoundBox(camera, canvas.getChildCanvas(true, false, 1, "normal", null, null), 0xFF0000);
				object._boundBox = containerBoundBox;
			}*/
