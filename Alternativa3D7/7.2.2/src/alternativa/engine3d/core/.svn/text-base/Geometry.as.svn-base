	...
	public class Geometry {
			...
			if (viewAligned) {
				...
				// Сортировка по средним Z
					...
					while (fragment != null) {
						...
						fragment.offset = sum/num;
						fragments[fragmentsLength] = fragment;
						fragmentsLength++;
						fragment = next;
					}
					// Сортировка
				...
				// Динамическое BSP
					...
					next = fragment.next;
					fragment.next = null;
					drawDynamicNode(fragment, next, threshold);
					fragment = null;
				}
				// Проецирование
				vertices.length = verticesLength;
		...
		private function drawStaticNode(splitter:Fragment):void {
			...
			if (splitter.offset < 0) {
				...
				if (splitter.num > 0) {
					while (splitter != null) {
						var next:Fragment = splitter.next;
						var indices:Vector.<int> = splitter.indices;
						var num:int = splitter.num;
						var a:int = indices[0];
						var b:int = indices[1];
