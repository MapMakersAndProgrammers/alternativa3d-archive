		...
		override alternativa3d function draw(camera:Camera3D, object:Object3D, parentCanvas:Canvas):void {
			...
			if (planeOccluderLength > 0) {
				...
				if (minSize > 0) {
					...
					var projectedEdgesLength:int = projectedEdges.length = (edgeOccluderLength/3) << 1;
				...
				camera.numOccluders++, planeOccluder.length = planeOccluderLength, edgeOccluder.length = edgeOccluderLength;
			} else {
				if (occludeAll) camera.numOccluders = 0, camera.occludedAll = true;
			}
			
			if (debug) {
				projectedEdges.length = (edgeOccluderLength/3) << 1;
				Utils3D.projectVectors(camera.projectionMatrix, edgeOccluder, projectedEdges, uvts);
				...
				if (edgeOccluderLength == 0 && occludeAll) {
