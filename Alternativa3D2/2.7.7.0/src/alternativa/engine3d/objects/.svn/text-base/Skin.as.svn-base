package alternativa.engine3d.objects {
	...
	public class Skin extends Mesh {
		...
		override protected function clonePropertiesFrom(source:Object3D):void {
			...
			var src:Skin = source as Skin;
			// Клонирование костей
			var vertex:Vertex;
			var value:Vertex;
			for (vertex = src.vertexList, value = vertexList; vertex != null; vertex = vertex.next, value = value.next) {
				vertex.value = value;
			}
			for (var i:int = 0; i < src._numJoints; i++) {
				joints[_numJoints++] = src.cloneJoint(src.joints[i]);
			}
			// Сброс после ремапа
