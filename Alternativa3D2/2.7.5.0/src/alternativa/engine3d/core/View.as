package alternativa.engine3d.core {
	...
	public class View extends Canvas {
		...
		private var pressedSource:Object3D;
		private var clickedTarget:Object3D;
		private var clickedSource:Object3D;
		private var overedTarget:Object3D;
		private var overedSource:Object3D;
		...
		private function onMouseDown(mouseEvent:MouseEvent):void {
			...
			defineTarget();
			if (target != null) {
				propagateEvent(MouseEvent3D.MOUSE_DOWN, target, source, branchToVector(target, branch));
			}
		...
		private function onMouseWheel(mouseEvent:MouseEvent):void {
			...
			defineTarget();
			if (target != null) {
				propagateEvent(MouseEvent3D.MOUSE_WHEEL, target, source, branchToVector(target, branch));
			}
			source = null;
			target = null;
		}
		...
		private function onClick(mouseEvent:MouseEvent):void {
			...
			defineTarget();
			if (target != null) {
				propagateEvent(MouseEvent3D.MOUSE_UP, target, source, branchToVector(target, branch));
				if (pressedTarget == target) {
					clickedTarget = target;
					clickedSource = source;
					propagateEvent(MouseEvent3D.CLICK, target, source, branchToVector(target, branch));
				}
			}
			pressedTarget = null;
			pressedSource = null;
		...
		private function onDoubleClick(mouseEvent:MouseEvent):void {
			...
			if (target != null) {
				propagateEvent(MouseEvent3D.MOUSE_UP, target, source, branchToVector(target, branch));
				if (pressedTarget == target) {
					propagateEvent(clickedTarget == target && target.doubleClickEnabled ? MouseEvent3D.DOUBLE_CLICK : MouseEvent3D.CLICK, target, source, branchToVector(target, branch));
				}
			}
			clickedTarget = null;
			clickedSource = null;
		...
		alternativa3d function onMouseMove(mouseEvent:MouseEvent = null):void {
			...
			if (mouseEvent != null && target != null) {
				propagateEvent(MouseEvent3D.MOUSE_MOVE, target, source, branchToVector(target, branch));
			}
			if (overedTarget != target) {
				...
				if (overedTarget != null) {
					propagateEvent(MouseEvent3D.MOUSE_OUT, overedTarget, overedSource, overedBranch, true, target, source);
					...
					if (changedBranchLength > 0) {
						...
						propagateEvent(MouseEvent3D.ROLL_OUT, overedTarget, overedSource, changedBranch, false, target, source);
					}
				}
				if (target != null) {
					changedBranchLength = 0;
					...
					if (changedBranchLength > 0) {
						changedBranch.length = changedBranchLength;
						propagateEvent(MouseEvent3D.ROLL_OVER, target, source, changedBranch, false, overedTarget, overedSource);
					}
					propagateEvent(MouseEvent3D.MOUSE_OVER, target, source, branch, true, overedTarget, overedSource);
				}
				overedTarget = target;
				overedSource = source;
			}
		...
		private function propagateEvent(type:String, target:Object3D, source:Object3D, objects:Vector.<Object3D>, bubbles:Boolean = true, relatedObject:Object3D = null, relatedSource:Object3D = null):void {
				...
				if (object.captureListeners != null) {
					...
					if (vector != null) {
						if (mouseEvent3D == null) {
							...
							mouseEvent3D.source = source;
							mouseEvent3D.relatedSource = relatedSource;
						}
...
class Logo extends Shape {
	...
	private	var w:int = 103;
	private	var h:int = 22;
	private	var d:int = 5;
	...
	public function Logo(view:View) {
		this.view = view;
		...
		super.graphics.beginBitmapFill(bmp, new Matrix(1, 0, 0, 1, -w - d, -h - d), false, true);
		super.graphics.drawRect(-w - d, -h - d, w, h);
		
		//super.graphics.endFill();
		//super.graphics.lineStyle(1, 0xFFFFFF);
		//super.graphics.drawCircle(0, 0, 100);
		//super.graphics.drawRect(-w*2, -h*2, w*2, h*2);
		
		addEventListener(Event.EXIT_FRAME, update, false);
		addEventListener(Event.ADDED_TO_STAGE, onAddToStage);
		addEventListener(Event.REMOVED_FROM_STAGE, onRemoveFromStage);
	}
	
	private function onAddToStage(e:Event):void {
		removeEventListener(Event.EXIT_FRAME, update, false);
		root.addEventListener(Event.EXIT_FRAME, update, false, -2147483648);
		stage.addEventListener(MouseEvent.MOUSE_DOWN, onMouseDown);
	...
	private function onRemoveFromStage(e:Event):void {
		addEventListener(Event.EXIT_FRAME, update, false);
		root.removeEventListener(Event.EXIT_FRAME, update, false);
		stage.removeEventListener(MouseEvent.MOUSE_DOWN, onMouseDown);
	...
	private function update(e:Event = null):void {
		if (stage != null) {
			if (old != null) {
				var step:Number = 10;
				var ax:Number = old.x - w - d + 1;
				var ay:Number = old.y - h - d + 1;
				var bx:Number = old.x - d - 1;
				var by:Number = old.y - d - 1;
				var total:int = 0;
				var right:int = 0;
				for (old.x = ax; old.x < bx; old.x += step) {
					for (old.y = ay; old.y < by; old.y += step) {
						total++
						var objects:Array = stage.getObjectsUnderPoint(old);
						if (objects[objects.length - 1] == this) right++;
					}
				}
				//trace(right, total, right/total);
				if (right/total < 0.8) view.clear();
			}
			...
			if (global.x < w + d*2) global.x = w + d*2;
			if (global.x > stage.stageWidth) global.x = stage.stageWidth;
			if (global.y < h + d*2) global.y = h + d*2;
			if (global.y > stage.stageHeight) global.y = stage.stageHeight;
			var local:Point = globalToLocal(global);
			var concatenatedMatrix:Matrix = super.transform.concatenatedMatrix;
			concatenatedMatrix.invert();
			concatenatedMatrix.tx = local.x;
			concatenatedMatrix.ty = local.y;
			super.transform.matrix = concatenatedMatrix;
			old = global;
		} else {
			super.x = view.width;
			super.y = view.height;
	...
	private function onMouseMove(e:MouseEvent):void {
		if (mouseX >= -w - d && mouseX < 0 && mouseY >= -h - d && mouseY < 0) {
			if (!on) {
				super.filters = [new GlowFilter(0xFFFF88, 1, 1.2, 1.2, 3, BitmapFilterQuality.HIGH)];
			}
			on = true;
		} else {
			if (on) {
	...
	override public function get filters():Array {return null;}
