package alternativa.engine3d.animation {
	...
	public class AnimationGroup extends Animation {
		 ...
		 */
		public function AnimationGroup(object:Transform3D = null, weight:Number = 1.0, speed:Number = 1.0) {
		...
		private function getObjectNumChildren(object:Transform3D):int {
			if (object is Skin) {
		...
		private function getObjectChildAt(object:Transform3D, index:int):Transform3D {
			if (object is Skin) {
		 ...
		 */
		private function getPathByTarget(target:Transform3D, base:Transform3D):Vector.<String> {
			var i:int, count:int;
			for (i = 0, count = getObjectNumChildren(base); i < count; i++) {
				var child:Transform3D = getObjectChildAt(base, i);
				if (child == target) {
		...
		private function getTargetByPath(path:Vector.<String>, base:Transform3D):Transform3D {
			var i:int, count:int;
			...
			for (i = 0, count = getObjectNumChildren(base); i < count; i++) {
				...
				if (child.name == null || child.name.length == 0) {
					// Ищем внутри
					var founded:Transform3D = getTargetByPath(path, child);
					if (founded != null) {
						return founded;
					}
				} else {
		 ...
		 */
		override public function reassign(object:Transform3D):void {
			if (object == null) {
				throw new ArgumentError("Object must be not null");
			}
			var base:Transform3D = this.object;
			if (base == null) {
			...
			for (i = 0; i < _numAnimations; i++) {
				...
				if (animation.object == base) {
					...
					var child:Transform3D = animation.object;
					if (child != null && child.name != null && child.name.length > 0) {
						...
						if (path != null) {
							var target:Transform3D = getTargetByPath(path, object);
							if (target != null) {
