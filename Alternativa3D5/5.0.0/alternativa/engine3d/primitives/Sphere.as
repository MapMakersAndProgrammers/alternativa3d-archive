package alternativa.engine3d.primitives {
	...
	public class Sphere extends Mesh {
		 ...
		 * Только установив <code>triangulate</code> в <code>true</code> вы сможете сделать это.</p>
		 * 
		...
		public function Sphere(radius:Number = 100, radialSegments:uint = 8, heightSegments:uint = 8, reverse:Boolean = false, triangulate:Boolean = false) {
			...
			var poleUp:Vertex = addVertex(0, 0, radius, "poleUp");
			var poleDown:Vertex = addVertex(0, 0, -radius, "poleDown");
			...
			for (segment = 1; segment < heightSegments; segment++) {
				...
				for (radial = 0; radial < radialSegments; radial++) {
					...
					addVertex(-Math.sin(currentRadialAngle)*segmentRadius, Math.cos(currentRadialAngle)*segmentRadius, segmentZ, radial + "_" + segment);
				}
			}

			// Создание граней и поверхности
			var surface:Surface = addSurface();
			...
			if (reverse) {
				for (radial = 0; radial < radialSegments; radial++) {
					...
					surface.addFace(addFace([poleUp, radial + "_1", prevRadial + "_1"], prevRadial + "_0"));
					// Грани нижнего полюса
					surface.addFace(addFace([radial + lastSegmentString, poleDown, prevRadial + lastSegmentString], prevRadial + lastSegmentString));
					...
					if (triangulate) {
						...
						for (segment = 1; segment < (heightSegments - 1); segment++) {
							...
							surface.addFace(addFace([radial + "_" + (segment + 1), prevRadial + "_" + (segment + 1), prevRadial + "_" + segment], prevRadial + "_" + segment + ":0"));
							surface.addFace(addFace([prevRadial + "_" + segment, radial + "_" + segment, radial + "_" + (segment + 1)], prevRadial + "_" + segment + ":1"));
						...
						// Грани середки
						for (segment = 1; segment < (heightSegments - 1); segment++) {
							surface.addFace(addFace([radial + "_" + (segment + 1), prevRadial + "_" + (segment + 1), prevRadial + "_" + segment, radial + "_" + segment], prevRadial + "_" + segment));
						}
					}
				...
				for (radial = 0; radial < radialSegments; radial++) {
					// Грани верхнего полюса
					surface.addFace(addFace([poleUp, prevRadial + "_1", radial + "_1"], prevRadial + "_0"));
					// Грани нижнего полюса
					surface.addFace(addFace([prevRadial + lastSegmentString, poleDown, radial + lastSegmentString], prevRadial + lastSegmentString));
					...
					if (triangulate) {
						...
						for (segment = 1; segment < (heightSegments - 1); segment++) {
							...
							surface.addFace(addFace([prevRadial + "_" + segment, prevRadial + "_" + (segment + 1), radial + "_" + (segment + 1)], prevRadial + "_" + segment + ":0"));
							surface.addFace(addFace([radial + "_" + (segment + 1), radial + "_" + segment, prevRadial + "_" + segment], prevRadial + "_" + segment + ":1"));
						...
						// Грани середки
						for (segment = 1; segment < (heightSegments - 1); segment++) {
							surface.addFace(addFace([prevRadial + "_" + segment, prevRadial + "_" + (segment + 1), radial + "_" + (segment + 1), radial + "_" + segment], prevRadial + "_" + segment));
						}
					}
					prevRadial = (radial == 0) ? 0: prevRadial + 1;
				}
