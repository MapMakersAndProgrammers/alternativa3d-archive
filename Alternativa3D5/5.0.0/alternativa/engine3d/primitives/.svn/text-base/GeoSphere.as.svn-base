package alternativa.engine3d.primitives {
	...
	public class GeoSphere extends Mesh	{
		...
		public function GeoSphere(radius:Number = 100, segments:uint = 2, reverse:Boolean = false) {
			...
			points.push(addVertex(0, 0, radius, "poleUp"));
			// Создание вершин верхней крышки
			for (i = 0; i < 5; i++) {
				theta = MathUtils.DEG360*i/5;
				sin = Math.sin(theta);
				cos = Math.cos(theta);
				points.push(addVertex(subrad*cos, subrad*sin, subz));
			}
			...
			points.push(addVertex(0, 0, -radius, "poleDown"));
			...
			for (f = 0; f < sections; f++) {
				for (var row:uint = 0; row < segments; row++) {
					for (var column:uint = 0; column <= row; column++) {
						...
						if (reverse) {
							face = addFace([va, vc, vb], (column << 1) + "_" + row + "_" + f);
							aUV.x = 1 - aUV.x;
							bUV.x = 1 - bUV.x;
							cUV.x = 1 - cUV.x;
							setUVsToFace(aUV, cUV, bUV, face);
						} else {
							face = addFace([va, vb, vc], (column << 1) + "_" + row + "_" + f);
						...
						//trace(a + "_" + b + "_" + c);
 						if (column < row) {
							...
							if (reverse) {
								face = addFace([va, vd, vc], ((column << 1) + 1) + "_" + row + "_" + f);
								aUV.x = 1 - aUV.x;
								bUV.x = 1 - bUV.x;
								cUV.x = 1 - cUV.x;
								setUVsToFace(aUV, bUV, cUV, face);
							} else {
								face = addFace([va, vc, vd], ((column << 1) + 1) + "_" + row + "_" + f);
								setUVsToFace(aUV, cUV, bUV, face);
							}
							surface.addFace(face);
						}
 ...
 */
		private function interpolate(v1:uint, v2:uint, num:uint, points:Array):void {
			...
			for (var e:uint = 1; e < num; e++) {
				...
				points.push(addVertex((a.x*st2 + b.x*st1)/sin, (a.y*st2 + b.y*st1)/sin, (a.z*st2 + b.z*st1)/sin));
			}
		}
