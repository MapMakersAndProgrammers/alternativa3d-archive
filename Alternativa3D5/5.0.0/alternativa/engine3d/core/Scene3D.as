package alternativa.engine3d.core {
	...
	public class Scene3D {
		...
		//private var operationSort:Array = ["priority", "level", "object", "name"];
		private var operationSort:Array = ["priority", "level"];
		private var operationSortOptions:Array = [Array.NUMERIC, Array.NUMERIC];
		// Вспомогательная пустая операция, используется при удалении операций из списка
		private var dummyOperation:Operation = new Operation("removed", this);
		
		// Флаг анализа сплиттеров
		private var _splitAnalysis:Boolean = true;
		// Cбалансированность дерева
		private var _splitBalance:Number = 0;
		/**
		 * @private
		...
		private var addSort:Array = ["mobility"];
		private var addSortOptions:Array = [Array.NUMERIC | Array.DESCENDING];
		private var addSplitQualitySort:Array = ["mobility", "splitQuality"];
		private var addSplitQualitySortOptions:Array = [Array.NUMERIC | Array.DESCENDING, Array.NUMERIC | Array.DESCENDING];
		/**
		 * @private
		 * Погрешность при определении точек на плоскости
		 */
		alternativa3d const planeOffsetTreshold:Number = 0.1;
		/**
		 * @private
		 * BSP-дерево
		 */
		...
		private var removeNodes:Set = new Set();
		// Вспомогательная пустая нода, используется при удалении нод из дерева
		private var dummyNode:BSPNode = new BSPNode();
		 ...
		 * видах, к которым подключены камеры, находящиеся в сцене.
		 */
		 ...
		 */
		private function calculateBSP():void {
					...
					if (i != j) { 
						...
						if (splitter.mobility <= primitive.mobility) {
							...
							for (var k:uint = 0; k < primitive.num; k++) {
								...
								if (pointOffset > planeOffsetTreshold) {
									if (!pointsFront) {
										splitter.disbalance++;
										pointsFront = true;
									}
		 ...
		 */
		private function addBSP(node:BSPNode, primitive:PolyPrimitive):void {
			var point:Point3D;
			var normal:Point3D;
			var key:*;
				...
				if (!pointsFront && !pointsBack) {
					...
					if (!pointsBack) {
						...
						if (!pointsFront) {
							...
							// Рассечение
							var fragment1:PolyPrimitive = PolyPrimitive.createPolyPrimitive();
							var fragment2:PolyPrimitive = PolyPrimitive.createPolyPrimitive();
							...
							for (i = 0; i < primitive.num; i++) {
								...
								if (offset1 > planeOffsetTreshold) {
									// Точка спереди плоскости ноды
									fragment2.points.push(points[i]);
									fragment2.uvs.push(primitive.uvs[i]);
								} else {
									if (offset1 < -planeOffsetTreshold) {
										// Точка сзади плоскости ноды
										fragment1.points.push(points[i]);
										fragment1.uvs.push(primitive.uvs[i]);
									} else {
								...
								// Рассечение ребра
								if (offset1 > planeOffsetTreshold && offset2 < -planeOffsetTreshold || offset1 < -planeOffsetTreshold && offset2 > planeOffsetTreshold) {
									// Находим точку рассечения
									var t:Number = offset1/(offset1 - offset2);
									point = Point3D.interpolate(points[i], points[j], t);
									fragment1.points.push(point);
									fragment2.points.push(point);
									...
									fragment1.uvs.push(uv);
									fragment2.uvs.push(uv);
							...
							fragment1.num = fragment1.points.length;
							fragment2.num = fragment2.points.length;
							
							// Назначаем мобильность
							fragment1.mobility = primitive.mobility;
							fragment2.mobility = primitive.mobility;

							// Устанавливаем связи рассечённых примитивов
							fragment1.face = primitive.face;
							fragment2.face = primitive.face;
							fragment1.parent = primitive;
							fragment2.parent = primitive;
							fragment1.sibling = fragment2;
							fragment2.sibling = fragment1;
							primitive.fragment1 = fragment1;
							primitive.fragment2 = fragment2;
							
							// Добавляем фрагменты в дочерние ноды 
							if (node.back == null) {
								node.back = BSPNode.createBSPNode(fragment1);
								node.back.parent = node;
								changedPrimitives[fragment1] = true;
							} else {
								addBSP(node.back, fragment1);
							}
							if (node.front == null) {
								node.front = BSPNode.createBSPNode(fragment2);
								node.front.parent = node;
								changedPrimitives[fragment2] = true;
							} else {
								addBSP(node.front, fragment2);
							}
						}
					}
				}
		 ...
		 */
		private function removeBSPNode(node:BSPNode):BSPNode {
		 ...
		 * @param node
		 */
		private function childBSP(node:BSPNode):void {
		 ...
		 */
		private function assembleChildPrimitives():void {
			var primitive:PolyPrimitive;
		...
		private function assemblePrimitive(primitive:PolyPrimitive):void {
			...
			if (primitive.sibling != null && canAssemble(primitive.sibling)) {
				...
				primitive.parent.fragment1 = null;
				primitive.parent.fragment2 = null;
		...
		private function canAssemble(primitive:PolyPrimitive):Boolean {
			if (childPrimitives[primitive]) {
				...
				var fragment1:PolyPrimitive = primitive.fragment1;
				var fragment2:PolyPrimitive = primitive.fragment2;
				if (fragment1 != null) {
					var assemble1:Boolean = canAssemble(fragment1);
					var assemble2:Boolean = canAssemble(fragment2);
					if (assemble1 && assemble2) {
						fragment1.parent = null;
						fragment2.parent = null;
						fragment1.sibling = null;
						fragment2.sibling = null;
						primitive.fragment1 = null;
						primitive.fragment2 = null;
						PolyPrimitive.destroyPolyPrimitive(fragment1);
						PolyPrimitive.destroyPolyPrimitive(fragment2);
						return true;
					} else {
						if (assemble1) {
							addPrimitives.push(fragment1);
						}
						if (assemble2) {
							addPrimitives.push(fragment2);
						}
					}
				}
			}
		...
		/**
		 * Флаг активности режима анализа сплиттеров. В режиме анализа для каждого добавляемого в BSP-дерево полигона выполняется
		 * его оценка в качестве разделяющей плоскости (сплиттера). Наиболее качественные сплиттеры добавляются в
		 * BSP-дерево первыми. Включив эту опцию вы сможете настроить качество и способ построения BSP дерева при помощи свойства splitBalance.
		 * 
		 * @see #splitBalance
		 * @default true
		 */
			...
			if (primitive != null) {
				if (primitive.face._surface != null && primitive.face._surface._material != null) {
					...
					if (primitive.face._surface._material is TextureMaterial) { 
						color = TextureMaterial(primitive.face._surface._material).texture.bitmapData.getPixel(0, 0);
					}
				}
			}
