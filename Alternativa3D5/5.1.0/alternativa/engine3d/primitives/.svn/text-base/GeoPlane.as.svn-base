		...
		public function GeoPlane(width:Number = 100, length:Number = 100, widthSegments:uint = 1, lengthSegments:uint = 1, twoSided:Boolean = true, reverse:Boolean = false) {
			...
			for (y = 0; y <= lengthSegments; y++) {
				...
				for (x = 0; x <= widthSegments; x++) {
					if ((y & 1) == 0) {
						// Если чётный ряд
						addVertex(x*ws - wh, y*hs - hh, 0, y + "_" + x);
						...
						if (x == 0) {
							// Первая точка
							addVertex(-wh, y*hs - hh, 0, y + "_" + x);
							frontUV[y][x] = new Point(0, y*vs);
							if (backUV != null) {
								backUV[y][x] = new Point(0, 1 - y*vs);
							}
						} else {
							addVertex(x*ws - wh - ws/2, y*hs - hh, 0, y + "_" + x);
							...
							if (x == widthSegments) {
								// Последняя точка
								addVertex(wh, y*hs - hh, 0, y + "_" + (x + 1));
			...
			if (twoSided || !reverse) {
				front = addSurface(null, "front");
			}
			if (twoSided || reverse) {
				back = addSurface(null, "back");
			}
				...
				for (var n:uint = 0; n <= (widthSegments << 1); n++) {
					...
					if ((y & 1) == 0) {
						...
						if ((n & 1) == 0) {
							// Если остриём вверх
							if (twoSided || !reverse) {
								face = addFace([y + "_" + x, (y + 1) + "_" + (x + 1), (y + 1) + "_" + x]);
								setUVsToFace(frontUV[y][x], frontUV[y + 1][x + 1], frontUV[y + 1][x], face); 
								front.addFace(face);
							}
							if (twoSided || reverse) {
								face = addFace([y + "_" + x, (y + 1) + "_" + x, (y + 1) + "_" + (x + 1)]);
								if (backUV != null) {
							...
							// Если остриём вниз 
							if (twoSided || !reverse) {
								face = addFace([y + "_" + x, y + "_" + (x + 1), (y + 1) + "_" + (x + 1)]);
								setUVsToFace(frontUV[y][x], frontUV[y][x + 1], frontUV[y + 1][x + 1], face);
								front.addFace(face);
							}
							if (twoSided || reverse) {
								face = addFace([y + "_" + x, (y + 1) + "_" + (x + 1), y + "_" + (x + 1)]);
								if (backUV != null) {
						...
						if ((n & 1) == 0) {
							// Если остриём вниз 
							if (twoSided || !reverse) {
								face = addFace([y + "_" + x, y + "_" + (x + 1), (y + 1) + "_" + x]);
								setUVsToFace(frontUV[y][x], frontUV[y][x + 1], frontUV[y + 1][x], face);
								front.addFace(face);
							}
							if (twoSided || reverse) {
								face = addFace([y + "_" + x, (y + 1) + "_" + x, y + "_" + (x + 1)]);
								if (backUV != null) {
							...
							// Если остриём вверх
							if (twoSided || !reverse) {
								face = addFace([y + "_" + (x + 1), (y + 1) + "_" + (x + 1), (y + 1) + "_" + x]);
								setUVsToFace(frontUV[y][x+1], frontUV[y + 1][x + 1], frontUV[y + 1][x], face);
								front.addFace(face);
							}
							if (twoSided || reverse) {
								face = addFace([y + "_" + (x + 1), (y + 1) + "_" + x, (y + 1) + "_" + (x + 1)]);
								if (backUV != null) {
