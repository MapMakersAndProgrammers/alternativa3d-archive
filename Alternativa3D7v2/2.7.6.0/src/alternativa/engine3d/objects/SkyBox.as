package alternativa.engine3d.objects {
	...
	public class SkyBox extends Mesh {
		...
		override public function clone():Object3D {
			var skyBox:SkyBox = new SkyBox(100);
			skyBox.cloneBaseProperties(this);
			skyBox.clipping = clipping;
			skyBox.sorting = sorting;
			skyBox.threshold = threshold;
			skyBox.vertexList = null;
			skyBox.faceList = null;
			// Клонирование вершин
			var vertex:Vertex;
			...
			for (var face:Face = faceList; face != null; face = face.next) {
				...
				if (lastFace != null) {
					...
					skyBox.faceList = newFace;
				}
				lastFace = newFace;
				// Клонирование отметок
				if (face == leftFace) {
					skyBox.leftFace = newFace;
				} else if (face == rightFace) {
					skyBox.rightFace = newFace;
				} else if (face == backFace) {
					skyBox.backFace = newFace;
				} else if (face == frontFace) {
					skyBox.frontFace = newFace;
				} else if (face == bottomFace) {
					skyBox.bottomFace = newFace;
				} else if (face == topFace) {
					skyBox.topFace = newFace;
				}
			}
			// Сброс после ремапа
			for (vertex = vertexList; vertex != null; vertex = vertex.next) {
