package alternativa.engine3d.objects {
	...
	public class Sprite3D extends Object3D {
		...
		override public function intersectRay(origin:Vector3D, direction:Vector3D, exludedObjects:Dictionary = null, camera:Camera3D = null):RayIntersectionData {
			if (camera == null || exludedObjects != null && exludedObjects[this]) return null;
			var vertex:Vertex;
			// Матрица перевода из камеры в объект
			camera.composeMatrix();
			var root:Object3D = camera;
			...
			vertex.u = 0;
			vertex.v = 0;
			face.wrapper.vertex = vertex;
			face.wrapper.next = new Wrapper();
			vertex = new Vertex();
			vertex.u = 0;
			vertex.v = 1;
			face.wrapper.next.vertex = vertex;
			face.wrapper.next.next = new Wrapper();
			vertex = new Vertex();
			vertex.u = 1;
			vertex.v = 1;
			face.wrapper.next.next.vertex = vertex;
			face.wrapper.next.next.next = new Wrapper();
			vertex = new Vertex();
			vertex.u = 1;
			vertex.v = 0;
			face.wrapper.next.next.next.vertex = vertex;
			var z:Number = ml;
			var size:Number;
			// Проекция
			...
			var ax:Number = ma/perspectiveScaleX;
			var ay:Number = me/perspectiveScaleY;
			var az:Number = mi;
			var bx:Number;
			var by:Number;
			var cx:Number;
			...
			if (rotation == 0) {
				...
				vertex.cameraY = ay;
				vertex.cameraZ = z;
				vertex = face.wrapper.next.vertex;
				vertex.cameraX = ax;
				vertex.cameraY = cy;
				vertex.cameraZ = z;
				vertex = face.wrapper.next.next.vertex;
				vertex.cameraX = cx;
				vertex.cameraY = cy;
				vertex.cameraZ = z;
				vertex = face.wrapper.next.next.next.vertex;
				vertex.cameraX = cx;
				vertex.cameraY = ay;
				vertex.cameraZ = z;
			} else {
				...
				vertex.cameraZ = z;
				vertex = face.wrapper.next.vertex;
				vertex.cameraX = ax + cameraHeightX;
				vertex.cameraY = ay + cameraHeightY;
				vertex.cameraZ = z;
				vertex = face.wrapper.next.next.vertex;
				vertex.cameraX = ax + cameraWidthX + cameraHeightX;
				vertex.cameraY = ay + cameraWidthY + cameraHeightY;
				vertex.cameraZ = z;
				vertex = face.wrapper.next.next.next.vertex;
				vertex.cameraX = ax + cameraWidthX;
				vertex.cameraY = ay + cameraWidthY;
				vertex.cameraZ = z;
			}
			...
			for (var wrapper:Wrapper = face.wrapper; wrapper != null; wrapper = wrapper.next) {
				...
				vertex.x = ma*vertex.cameraX + mb*vertex.cameraY + mc*vertex.cameraZ + md;
				vertex.y = me*vertex.cameraX + mf*vertex.cameraY + mg*vertex.cameraZ + mh;
				vertex.z = mi*vertex.cameraX + mj*vertex.cameraY + mk*vertex.cameraZ + ml;
			}
			// Расчёт нормали
			...
			if (dot < 0) {
				var offset:Number = origin.x*face.normalX + origin.y*face.normalY + origin.z*face.normalZ - face.offset;
				if (offset > 0) {
					var dst:Number = -offset/dot;
					cx = origin.x + direction.x*dst;
					cy = origin.y + direction.y*dst;
					cz = origin.z + direction.z*dst;
					for (wrapper = face.wrapper; wrapper != null; wrapper = wrapper.next) {
						...
						acy = cy - a.y;
						acz = cz - a.z;
						if ((acz*aby - acy*abz)*face.normalX + (acx*abz - acz*abx)*face.normalY + (acy*abx - acx*aby)*face.normalZ < 0) break;
					}
					if (wrapper == null) {
						var point:Vector3D = new Vector3D(cx, cy, cz);
						...
						ay = point.y - origin.y;
						az = point.z - origin.z;
						res.time = Math.sqrt(ax*ax + ay*ay + az*az);
						return res;
					}
				}
			}
