	...
	public class BSPContainer extends ConflictContainer	{
		...
		public function createTree(staticGeometry:Vector.<Geometry>, staticObjects:Vector.<Object3D> = null, splitters:Vector.<Geometry> = null):void {
			...
			if (splitters != null) {
				...
				for (i = len - 1; i >= 0; i--) {
					faceList = calculateFaceList(splitters[i], false, splitterList);
				}
			}
			// Обработка геометрии
			len = staticGeometry.length;
			for (i = len - 1; i >= 0; i--) {
				faceList = calculateFaceList(staticGeometry[i], true, faceList);
			}
			// Обработка объектов
		...
		override public function intersectRay(origin:Vector3D, direction:Vector3D, exludedObjects:Dictionary = null, camera:Camera3D = null):RayIntersectionData {
			...
			for (var child:Object3D = childrenList; child != null; child = child.next) {
				...
				data = child.intersectRay(childOrigin, childDirection, exludedObjects, camera);
				if (data != null && data.time < minTime) {
					minTime = data.time;
					res = data;
				}
			}
			if (root != null && boundIntersectRay(origin, direction, root.boundMinX, root.boundMinY, root.boundMinZ, root.boundMaxX, root.boundMaxY, root.boundMaxZ)) {
				data = intersectRayNode(root, origin, direction, exludedObjects, camera);
				if (data != null && data.time < minTime) {
					minTime = data.time;
					res = data;
				}
		...
		private function intersectRayNode(node:BSPNode, origin:Vector3D, direction:Vector3D, exludedObjects:Dictionary, camera:Camera3D):RayIntersectionData {
			var data:RayIntersectionData;
			// Узловая нода
			if (node.objectList == null) {
				var offset:Number = node.normalX*origin.x + node.normalY*origin.y + node.normalZ*origin.z - node.offset;
				if (offset > 0) {
					if (node.positive != null && boundIntersectRay(origin, direction, node.positive.boundMinX, node.positive.boundMinY, node.positive.boundMinZ, node.positive.boundMaxX, node.positive.boundMaxY, node.positive.boundMaxZ)) {
						data = intersectRayNode(node.positive, origin, direction, exludedObjects, camera);
						if (data != null) return data;
					}
					var dot:Number = direction.x*node.normalX + direction.y*node.normalY + direction.z*node.normalZ;
					if (dot < 0) {
						var time:Number = -offset/dot;
						var cx:Number = origin.x + direction.x*time;
						var cy:Number = origin.y + direction.y*time;
						var cz:Number = origin.z + direction.z*time;
						...
						if (node.negative != null && boundIntersectRay(origin, direction, node.negative.boundMinX, node.negative.boundMinY, node.negative.boundMinZ, node.negative.boundMaxX, node.negative.boundMaxY, node.negative.boundMaxZ)) {
							return intersectRayNode(node.negative, origin, direction, exludedObjects, camera);
						}
					}
				} else {
					if (node.negative != null && boundIntersectRay(origin, direction, node.negative.boundMinX, node.negative.boundMinY, node.negative.boundMinZ, node.negative.boundMaxX, node.negative.boundMaxY, node.negative.boundMaxZ)) {
						data = intersectRayNode(node.negative, origin, direction, exludedObjects, camera);
						if (data != null) return data;
					}
					if (node.positive != null && direction.x*node.normalX + direction.y*node.normalY + direction.z*node.normalZ > 0 && boundIntersectRay(origin, direction, node.positive.boundMinX, node.positive.boundMinY, node.positive.boundMinZ, node.positive.boundMaxX, node.positive.boundMaxY, node.positive.boundMaxZ)) {
						return intersectRayNode(node.positive, origin, direction, exludedObjects, camera);
					}
				}
			// Конечная нода
			} else {
		...
		override public function clone():Object3D {
			var res:BSPContainer = new BSPContainer();
		...
		override protected function clonePropertiesFrom(source:Object3D):void {
			...
			var src:BSPContainer = source as BSPContainer;
			clipping = src.clipping;
			debugAlphaFade = src.debugAlphaFade;
			// Клонирование вершин
			var vertex:Vertex;
			...
			// Клонирование дерева
			if (src.root != null) {
				root = src.cloneNode(src.root, this);
			}
			// Сброс после ремапа
			for (vertex = src.vertexList; vertex != null; vertex = vertex.next) {
				vertex.value = null;
		...
		private function cloneNode(node:BSPNode, parent:Object3DContainer):BSPNode {
			var newNode:BSPNode = new BSPNode();
			...
			var object:Object3D;
			var lastObject:Object3D;
			var newObject:Object3D;
			for (object = node.objectList, lastObject = null; object != null; object = object.next) {
			...
			for (object = node.boundList, lastObject = null; object != null; object = object.next) {
				newObject = object.clone();
				if (newNode.boundList != null) {
					lastObject.next = newObject;
				} else {
					newNode.boundList = newObject;
				}
				lastObject = newObject;
			}
			if (node.negative != null) newNode.negative = cloneNode(node.negative, parent);
			if (node.positive != null) newNode.positive = cloneNode(node.positive, parent);
			return newNode;
		}
		...
		override alternativa3d function getVG(camera:Camera3D):VG {
			...
			if (root != null) {
				...
				calculateInverseMatrix();
				calculateCameraPlanes(camera.nearClipping, camera.farClipping);
				rootCulling = cullingInContainer(culling, root.boundMinX, root.boundMinY, root.boundMinZ, root.boundMaxX, root.boundMaxY, root.boundMaxZ);
				if (rootCulling >= 0) res = collectVGNode(root, rootCulling, camera, res);
			}
			colorizeVG(res);
			/*if (root != null) {
				var tree:Face = prepareNode(root, rootCulling, camera);
				if (tree != null) {
					var vg:VG = VG.create(this, tree, 3, camera.debug ? camera.checkInDebug(this) : 0, false);
					vg.next = res;
					res = vg;
				}
			}*/
		...
		private function collectVGNode(node:BSPNode, culling:int, camera:Camera3D, result:VG = null):VG {
			var first:VG;
			var last:VG;
			for (var child:Object3D = node.objectList, bound:Object3D = node.boundList; child != null; child = child.next, bound = bound.next) {
			...
			if (node.negative != null) {
				...
				if (negativeCulling >= 0) result = collectVGNode(node.negative, negativeCulling, camera, result);
			}
			if (node.positive != null) {
				...
				if (positiveCulling >= 0) result = collectVGNode(node.positive, positiveCulling, camera, result);
			}
			return result;
		}
		
		/*private function prepareNode(node:BSPNode, culling:int, camera:Camera3D):Face {
			var list:Face;
			var w:Wrapper;
		...
		override alternativa3d function updateBounds(bounds:Object3D, transformation:Object3D = null):void {
			...
			if (root != null) updateBoundsNode(root, bounds, transformation);
		}
		
		private function updateBoundsNode(node:BSPNode, bounds:Object3D, transformation:Object3D):void {
			if (node.objectList == null) {
				if (node.negative != null) updateBoundsNode(node.negative, bounds, transformation);
				if (node.positive != null) updateBoundsNode(node.positive, bounds, transformation);
			} else {
				for (var child:Object3D = node.objectList; child != null; child = child.next) {
					if (transformation != null) {
		...
		private function calculateFaceList(geometry:Geometry, saveVertices:Boolean, result:Face = null):Face {
			var faceList:Face;
			var lastFace:Face;
			var i:int;
			var vertex:Vertex;
		...
		private function createNode(splitterList:Face, faceList:Face, objectList:Object3D, boundList:Object3D, faceSplitResult:Vector.<Face>, objectSplitResult:Vector.<Object3D>, parentSplitter:Face = null):BSPNode {
			...
			if (splitterList != null) {
				...
				if (objectList != null) {
					...
					for (face = parentSplitter; face != null; face = face.next) {
						boundFaceList = cropBoundFaceList(boundFaceList, face.normalX, face.normalY, face.normalZ, face.offset);
			...
			if (splitter != null) {
				...
				if (splitterList != null) {
					splitFaceList(splitter, splitterList, false, faceSplitResult);
					negativeSplitterList = faceSplitResult[0];
					positiveSplitterList = faceSplitResult[2];
				}
				if (faceList != null) {
					splitFaceList(splitter, faceList, true, faceSplitResult);
		...
		private function cropBoundFaceList(faceList:Face, normalX:Number, normalY:Number, normalZ:Number, offset:Number):Face {
			var res:Face;
			var offsetMin:Number = offset - threshold;
			var offsetMax:Number = offset + threshold;
		...
		private function splitFaceList(splitter:Face, faceList:Face, saveVertices:Boolean, faceSplitResult:Vector.<Face>):void {
