package alternativa.engine3d.containers {
	...
	public class KDContainer extends ConflictContainer {
		...
		private function intersectRayNode(node:KDNode, origin:Vector3D, direction:Vector3D, exludedObjects:Dictionary, camera:Camera3D):RayIntersectionData {
			var data:RayIntersectionData;
			var dot:Number;
			var child:Object3D;
			var bound:Object3D;
			...
			if (node.negative != null) {
				...
				var offset:Number = (axisX ? origin.x : (axisY ? origin.y : origin.z)) - node.coord;
				if (offset > 0) {
		...
		private function cloneNode(node:KDNode, parent:Object3DContainer):KDNode {
			...
			for (object = node.objectBoundList, lastObject = null; object != null; object = object.next) {
				newObject = object.clone();
			...
			for (object = node.occluderList, lastObject = null; object != null; object = object.next) {
				newObject = object.clone();
			...
			for (object = node.occluderBoundList, lastObject = null; object != null; object = object.next) {
				newObject = object.clone();
		...
		private function collectVGNode(node:KDNode, culling:int, camera:Camera3D, result:VG = null):VG {
			var first:VG;
			var last:VG;
			for (var child:Object3D = node.objectList, bound:Object3D = node.objectBoundList; child != null; child = child.next, bound = bound.next) {
		...
		private function updateBoundsNode(node:KDNode, bounds:Object3D, transformation:Object3D):void {
			...
			if (node.negative != null) {
				updateBoundsNode(node.negative, bounds, transformation);
				updateBoundsNode(node.positive, bounds, transformation);
			}
		}
