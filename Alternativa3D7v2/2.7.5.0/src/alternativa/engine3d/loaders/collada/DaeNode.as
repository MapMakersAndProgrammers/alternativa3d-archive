	...
	public class DaeNode extends DaeElement {
		...
		public function parseSkins():Vector.<DaeObject> {
			...
			for (var i:int = 0, count:int = instanceControllers.length; i < count; i++) {
				...
				if (controller != null) {
					controller.parse();
					var animatedSkinAndJoints:DaeObject = controller.parseSkin(this, parseInstanceMaterials(instanceController.data), instanceController.skeletons);
					if (animatedSkinAndJoints != null) {
						...
						var animatedSkin:DaeObject = applyAnimation(applyTransformations(skin));
						if (animatedSkin.animation != null) {
							var animatedSkinAndJointsAnimation:AnimationGroup = animatedSkinAndJoints.animation as AnimationGroup;
							if (animatedSkinAndJointsAnimation != null) {
								var animatedSkinAnimation:AnimationGroup = animatedSkin.animation as AnimationGroup;
								if (animatedSkinAnimation == null) {
									animatedSkinAnimation = new AnimationGroup(skin);
									animatedSkinAnimation.addAnimation(animatedSkin.animation);
									animatedSkin.animation = animatedSkinAnimation;
								}
								// Переносим анимации
								for (var a:int = 0, numAnimations:int = animatedSkinAndJointsAnimation.numAnimations; a < numAnimations; a++) {
									animatedSkinAnimation.addAnimation(animatedSkinAndJointsAnimation.getAnimationAt(a));
								}
							}
						} else {
				...
				for (var i:int = 0, count:int = children.length(); i < count; i++) {
					...
					switch (child.localName()) {
						...
						case "instance_geometry":
							...
							if (geom != null) {
								geom.parse();
								var daeObject:DaeObject;
								var mesh:Mesh = geom.parseMesh(parseInstanceMaterials(child));
								if (mesh != null) {
									if (mesh.sorting == 3) {
										var bsp:BSP = new BSP();
										bsp.splitAnalysis = mesh.transformId > 0;
										bsp.geometry = mesh.geometry;
										bsp.name = getNewName(objects.length);
										daeObject = applyAnimation(applyTransformations(bsp));
										daeObject.isSplitter = geom.isSplitter;
										daeObject.isStaticGeometry = geom.isBaseGeometry;
										objects.push(daeObject);
									} else {
										if (mesh.sorting == Sorting.DYNAMIC_BSP && mesh.transformId > 0) {
											mesh.optimizeForDynamicBSP();
										}
										mesh.name = getNewName(objects.length);
										daeObject = applyAnimation(applyTransformations(mesh));
										daeObject.isSplitter = geom.isSplitter;
										daeObject.isStaticGeometry = geom.isBaseGeometry;
										objects.push(daeObject);
									}
...
//			}
		 ...
		 */
		public function applyAnimation(object:Transform3D, animation:Animation = null):DaeObject {
		 ...
		 */
		public function applyTransformations(object:Transform3D, prepend:Matrix3D = null, append:Matrix3D = null):Transform3D {
			if (append != null) {
				var matrix:Matrix3D = getMatrix(prepend);
				matrix.append(append);
				object.matrix = matrix;
			} else {
				object.matrix = getMatrix(prepend);
			}
			return object;
		}
		...
		private function parseAlternativa3DObject():Object3D {
			var profile:XML = data.extra.technique.(@profile == "Alternativa3D").instance[0];
			if (profile != null) {
