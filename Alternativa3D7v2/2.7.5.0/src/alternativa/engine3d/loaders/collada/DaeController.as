package alternativa.engine3d.loaders.collada {
	...
	public class DaeController extends DaeElement {
		 ...
		 */
		public function parseSkin(skinNode:DaeNode, materials:Object, skeletons:Vector.<DaeNode>):DaeObject {
			...
			if (skinXML != null) {
				...
				if (geom != null) {
					...
					var joints:Vector.<DaeObject> = addJointsToSkin(skin, skinNode,  findNodes(skeletons));
					...
					if (skin.sorting == Sorting.DYNAMIC_BSP && skin.transformId > 0) {
						skin.optimizeForDynamicBSP();
					}
		 ...
		 */
		private function addJointsToSkin(skin:Skin, skinNode:DaeNode, nodes:Vector.<DaeNode>):Vector.<DaeObject> {
			...
			for (i = 0; i < count; i++) {
				...
				if (isRootJointNode(node, nodesDictionary)) {
					var animatedJoint:DaeObject = addRootJointToSkin(skin, skinNode, node);
		 ...
		 */
		private function addRootJointToSkin(skin:Skin, skinNode:DaeNode, node:DaeNode):DaeObject {
			var joint:Joint;
			if (skinNode == node) {
				...
				if (node.scene == skinNode.scene) {
					...
					additionalJoint.matrix = toSkinMatrix;
