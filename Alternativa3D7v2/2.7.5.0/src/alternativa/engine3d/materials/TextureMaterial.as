package alternativa.engine3d.materials {
	...
	/**
	 * Материал, заполняющий полигон текстурой.
	 */
	public class TextureMaterial extends Material {
		 ...
		 * @private 
		 */
		static alternativa3d var drawVertices:Vector.<Number> = new Vector.<Number>();
		
		/**
		 * @private 
		 */
		static alternativa3d var drawUVTs:Vector.<Number> = new Vector.<Number>();
		
		/**
		 * @private 
		 */
		static alternativa3d var drawIndices:Vector.<int> = new Vector.<int>();
		...
		/**
		 * Разрешение текстуры.
		 * Отношение, сколько единиц трёхмерного пространства приходится на пиксел текстуры.
		 * Значение имеет смысл только при значении <code>mipMapping</code> равном <code>MipMapping.OBJECT_DISTANCE</code> или <code>MipMapping.PER_PIXEL</code>.
		 * Для расчёта <code>resolution</code> для конкретного трёхмерного объекта можно использовать его метод <code>calculateResolution()</code>.
		 ...
		 * Учитывается при значении <code>mipMapping</code> равном <code>MipMapping.PER_PIXEL</code>.
		...
		/**
		 * Флаг коррекции текстурных координат.
		 ...
		 * @param mipMapping Режим мипмаппинга.
		 * @param resolution Разрешение текстуры.
		 * @see alternativa.engine3d.core.MipMapping
		 */
		...
		/**
		 * Режим мипмаппинга.
		 * Можно использовать константы класса <code>MipMapping</code>: <code>MipMapping.NONE</code>, <code>MipMapping.OBJECT_DISTANCE</code>, <code>MipMapping.PER_PIXEL</code>. 
		 * @see alternativa.engine3d.core.MipMapping
		 */
		...
		private function calculateMipMaps():void {
			mipMap = new Vector.<BitmapData>();
			numMaps = 0;
			mipMap[numMaps] = _texture;
			numMaps++;
			if (_texture.width % 2 > 0 || _texture.height % 2 > 0) return;
					...
					for (face = list,list = null,last = null; face != null; face = next) {
						...
						if (wrapper != null) {
							...
							if (temporaryWrapper != null) {
								for (wrapper = temporaryWrapper; wrapper != null; wrapper = wrapper.next) wrapper.vertex = null;
