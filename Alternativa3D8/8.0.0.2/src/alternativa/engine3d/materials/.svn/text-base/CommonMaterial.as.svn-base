package alternativa.engine3d.materials {
	...
	import flash.display.TextureCube3D;
	...
	public class CommonMaterial extends Material {
		...
		private function initMeshProgram(camera:Camera3D, view:Bitmap3D, makeShadows:Boolean):Program3D {
			 ...
			 * [shadows==true] vc12 - vc15 - lighting
			 */
			var vertexShader:String = 
				...
				if (shadows) {
					vertexShader += light.attenuateVertexShader("v3", 12);
				}
				if (ambientCube != null) {
					vertexShader +=
					"dp3 vt0.x, va3, vc8 \n" +		// vt0 - world normal
			 ...
			 * [shadows==true] fc2 - lighting constant
			 * fc3 : light color
			 * ft0 - result
			 */
			var fragmentShader:String = 
				...
				if (shadows) {
					fragmentShader += light.attenuateFragmentShader("ft5", "v3", 5, 2) +		// ft5 - shadow
...
//				"div ft2.x, ft2.x, ft2.y \n";
		...
		override alternativa3d function drawMesh(mesh:Mesh, camera:Camera3D):void {
			...
			if (emissionChannel > 0) {
				view.setVertexStream(4, mesh.geometry.additionalUVChannels, 0, "FLOAT_2"); 
			}
			view.setProgramConstants("FRAGMENT", 0, 1, Vector.<Number>([0.5, glossiness, ambient, 1]));
			if (makeShadows) {
				var light:DirectionalLight = camera.lights[0];
				if (shadows) {
					light.attenuateProgramSetup(view, mesh, 12, 5, 2);
				}
...
//					view.setProgramConstants("FRAGMENT", 1, 1, Vector.<Number>([direction.x, direction.y, direction.z, 1]));
				...
				view.setProgramConstants("FRAGMENT", 3, 1, light.color);
			} else {
			...
			if (ambientCube != null) {
				...
				toWorld.invert();
				view.setProgramConstantsMatrix("VERTEX", 8, toWorld);
				view.setTexture(6, ambientCube);
			}
			...
			if (emissionChannel > 0) {
				view.setVertexStream(4, null, 0, "DISABLED");
			}
			if (opacity3D != null) {
