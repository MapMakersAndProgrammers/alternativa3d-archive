...
//					projectionMatrixData[13] = 0;
				...
				if (root != this && root.visible) {
					numLights = (lights == null) ? 0 : lights.length;
					if (numLights > 0 && !lights[0].shadow) {
						numLights = 0;
					}
					if (numLights > 0) {
...
//								nearClipping = light.currentSplitNear;
//								farClipping = light.currentSplitFar;
								...
								if (root.cullingInCamera(this, 63) >= 0) {
									root.draw(this);
									if (sortTransparentObjects) sortByDistance();
