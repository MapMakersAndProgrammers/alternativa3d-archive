package alternativa.engine3d.core {
	...
	public class Geometry {
		...
		alternativa3d var additionalUVChannels:VertexBuffer3D;
		alternativa3d var numAdditionalUVChannels:int = 0;

		/**
		 * Список дополнительных ув канналов, 1 каннал сохраняется по нулевому индексу в массиве. 
		 */
		alternativa3d var uvChannels:Array;
		
		public var hasTangents:Boolean = false;
		public var hasNormals:Boolean = false;

		public function getUVChannel(index:uint):Vector.<Point> {
			if (index == 0) {
				// Возвращаем uv из вершин
				var vertices:Vector.<Vertex> = orderedVertices;
				var numVertices:int = vertices.length;
				var uvs:Vector.<Point> = new Vector.<Point>(numVertices);
				for (var i:int = 0; i < numVertices; i++) {
					var vertex:Vertex = vertices[i];
					uvs[i] = new Point(vertex._u, vertex._v);
				}
				return uvs;
			}
			return (uvChannels != null) ? uvChannels[int(index - 1)] : null;
		}

		public function setUVChannel(index:uint, values:Vector.<Point>):void {
			if (index == 0) {
				var vertices:Vector.<Vertex> = orderedVertices;
				var numVertices:int = vertices.length;
				...
				if (uvChannels[int(index - 1)] == null) numAdditionalUVChannels++;
				uvChannels[int(index - 1)] = new Vector.<Point>().concat(values);
				additionalUVChannels = null;
			}
		}

		alternativa3d function update(context3d:Context3D):void {
			if (indexBuffer != null && (numAdditionalUVChannels == 0 || additionalUVChannels != null)) return;
			numTriangles = 0;
			...
			if (numAdditionalUVChannels > 0 && additionalUVChannels == null) {
				// Выгрузка uv канналов
				var uvs:Vector.<Number> = new Vector.<Number>(2*orderedVerticesLength*numAdditionalUVChannels);
				var channelIndex:int = 0;
				for each (var channel:Vector.<Point> in uvChannels) {
					var numUVs:int = channel.length;
					for (i = 0; i < orderedVerticesLength; i++) {
						var uvIndex:int = 2 * (numAdditionalUVChannels * i + channelIndex); 
						if (i < numUVs) {
							var point:Point = channel[i];
							if (point != null) {
								uvs[uvIndex] = point.x;
								uvs[int(uvIndex + 1)] = point.y;
							}
						}
					}
					channelIndex++;
				}
				additionalUVChannels = context3d.createVertexBuffer(orderedVerticesLength, 2 * numAdditionalUVChannels);
				additionalUVChannels.upload(uvs, 0, orderedVerticesLength);
			...
			maxAttributes = maxAttributes + numJointsInGeometry + (hasTangents ? 12 : (hasNormals ? 8 : 5)); // x,y,z,u,v отдельно
			...
			vertexBuffer = context3d.createVertexBuffer(orderedVerticesLength, maxAttributes);
			vertexBuffer.upload(vertices, 0, orderedVerticesLength);
			indexBuffer = context3d.createIndexBuffer(faces.length);
...
//				newFace.material = face.material;
		...
		alternativa3d function group(verts:Vector.<Vertex>, begin:int, end:int, depth:int, distanceThreshold:Number, uvThreshold:Number, stack:Vector.<int>):void {
			var i:int;
			var j:int;
			var vertex:Vertex;
		...
		public function calculateTBNs(smoothSplits:Boolean = false, removeJointsIndicesAndWeights:Boolean = true):void {
			var vertex:Vertex;
			if (smoothSplits) {
				// Заполнение массива вершин
				var verts:Vector.<Vertex> = new Vector.<Vertex>();
				...
				// Группировка
				group(verts, 0, vertsLength, 0, 0, Number.MAX_VALUE, new Vector.<int>());
			}
...
//				var vertex:Vertex = key;
				...
				// выполняем ортогонализацию между нормалью и тангентом			
				...
				// находим направление бинормали
				var crossX:Number = normalY*tangentX - normalZ*tangentY;
        		...
        		if (removeJointsIndicesAndWeights) {
	        		vertex._jointsIndices = null;
	        		vertex._jointsWeights = null;
	        	}
        		
//				vertex._attributes = Vector.<Number>([tangentX/tangentLen, tangentY/tangentLen, tangentZ/tangentLen, tangentW, normalX, normalY, normalZ]);
				vertex.normal = new Vector3D(normalX, normalY, normalZ);
				vertex.tangent = new Vector3D(tangentX/tangentLen, tangentY/tangentLen, tangentZ/tangentLen, tangentW);
			}
			if (smoothSplits) {
				// Установка тангенсов дубликатам
				for (var i:int = 0; i < vertsLength; i++) {
					vertex = verts[i];
					if (vertex.value != null) {
						if (vertex.value.normal != null) {
							// Вершина принадлежит хотя бы одной грани
						//	vertex._attributes = new Vector.<Number>().concat(vertex.value._attributes);
							vertex.normal = vertex.value.normal.clone();
							
						}
						if (vertex.value.tangent != null) {
							vertex.tangent = vertex.value.tangent.clone();
						}
						vertex.value = null;
					}
				}
