package alternativa.engine3d.loaders.collada {
	...
	public class DaePrimitive extends DaeElement {
			...
			switch (this.type) {
				case "polygons" : {
					...
					for (i = 0; i < count; i++) {
						...
						fillInPolygon(geometry, material, vertices, verticesInput.offset, indices.length/inputsStride, indices, normals, texCoordsDatas);
					}
					break;
				}
				case "polylist" : {
					...
					if (vcountsXML != null) {
						...
						fillInPolylist(geometry, material, vertices, verticesInput.offset, numPrimitives, indices, vcounts, normals, texCoordsDatas);
					} else {
						fillInPolygon(geometry, material, vertices, verticesInput.offset, numPrimitives, indices, normals, texCoordsDatas);
					}
					break;
				}
				case "triangles" : {
					...
					indices = parseIntsArray(indicesXML);
					fillInTriangles(geometry, material, vertices, verticesInput.offset, numPrimitives, indices, normals, texCoordsDatas);
					break;
				}
			}
		}
		
		private function fillNormals(normalsSource:DaeSource):Vector.<Vector3D> {
			var values:Vector.<Number> = normalsSource.numbers;
			var stride:int = normalsSource.stride;
			var count:int = values.length/normalsSource.stride;
			var normals:Vector.<Vector3D> = new Vector.<Vector3D>(count);
			for (var i:int = 0; i < count; i++) {
				var normal:Vector3D = new Vector3D();
				var index:int = i*stride;
				normal.x = values[index];
				normal.y = values[index + 1];
				normal.z = values[index + 2];
				normals[i] = normal;
			}
			return normals;
		}
		 ...
		 */
		private function fillInPolygon(geometry:Geometry, material:Material, vertices:Vector.<Vertex>, verticesOffset:int, numIndices:int, indices:Array, normals:Vector.<Vector3D>, texCoordsDatas:Vector.<TexCoordsData> = null):void {
			...
			for (var i:int = 0; i < numIndices; i++) {
				...
				if (normalsInput) {
					...
					vertex.normal = normals[indices[normalIndex]];
				}
				if (texCoordsDatas != null) {
		...
		private function fillInPolylist(geometry:Geometry, material:Material, vertices:Vector.<Vertex>, verticesOffset:int, numFaces:int, indices:Array, vcounts:Array, normals:Vector.<Vector3D>, texCoordsDatas:Vector.<TexCoordsData> = null):void {
			var polyIndex:int = 0;
			for (var i:int = 0; i < numFaces; i++) {
				...
				if (count >= 3) {
					...
					for (var j:int = 0; j < count; j++) {
						...
						if (normalsInput) {
							var normalIndex:int = vertexIndex + normalsInput.offset;
							vertex.normal = normals[indices[normalIndex]];
						}
						if (texCoordsDatas != null) {
							var numTexCoordsDatas:int = texCoordsDatas.length;
		...
		private function fillInTriangles(geometry:Geometry, material:Material, vertices:Vector.<Vertex>, verticesOffset:int, numFaces:int, indices:Array, normals:Vector.<Vector3D>, texCoordsDatas:Vector.<TexCoordsData> = null):void {
			...
			for (var i:int = 0; i < numFaces; i++) {
				...
				if (normalsInput) {
					var normalIndex:int = index + normalsInput.offset;
					a.normal = normals[indices[normalIndex]];
					b.normal = normals[indices[normalIndex + inputsStride]];
					c.normal = normals[indices[normalIndex + 2*inputsStride]];
