package alternativa.engine3d.core {
	...
	public class Geometry {
		...
		public var needCalculateNormals:Boolean = true;
		...
		alternativa3d function update(view:Bitmap3D):void {
			...
			if (numAdditionalUVChannels > 0 && additionalUVChannels == null) {
				...
				additionalUVChannels = view.createVertexBuffer(orderedVerticesLength, 2 * numAdditionalUVChannels);
...
//				var vertex:Vertex = key;
			...
			if (smoothSplits) {
				...
				for (var i:int = 0; i < vertsLength; i++) {
					vertex = verts[i];
					if (vertex.value != null) {
						if (vertex.value._attributes != null) {
