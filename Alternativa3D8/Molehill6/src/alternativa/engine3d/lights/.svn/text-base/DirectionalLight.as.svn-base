	...
	public class DirectionalLight extends Object3D {
		...
		alternativa3d function update(camera:Camera3D, view:View, splitIndex:int):void {
			...
			if (debugShadowMaterial != null) {
				debugShadowMaterial.update(view);
			}
		...
		alternativa3d function predraw(view:View):void {
			if (!currentSplitHaveShadow) {
				// Создаем или очищаем шедоумапу
				if (createTextures || shadowMap == null) {
					if (useSingle) {
//						shadowMap = view.createTexture(_width, _height, "SINGLE", true);
						shadowMap = view.createTexture(_width, _height, "SINGLE_FLOAT", true);
					} else {
...
//			trace("SET:", texture);
			...
			if (debugShadowMaterial != null) {
				debugShadowMaterial.evaluateProgramSetupSimple(view, constF2, texture);
			} else {
				view.setProgramConstants("FRAGMENT", constF2, 1, options);
			}
		}

		alternativa3d function attenuateProgramClean(view:View, texture:uint):void {
//			trace("CLN:", texture);
//			if (shadow && !createTextures) {
			if (!createTextures) {
				if (dummy == null) {
					dummy = view.createTexture(1, 1, "BGRA", false);
				}
				view.setTexture(texture, dummy);
			}
		}
