package alternativa.engine3d.core {
	...
	public class Camera3D extends Object3D {
		...
		/**
		 * Контекст камеры.
		 * Если не указан, отрисовка осуществляться не будет.
		 */
		public var context3d:Context3D;
		
		/**
		 * Ширина используемая для расчета настроек камеры 
		 */
		public var width:Number = 0;
		/**
		 * Высота используемая для расчета настроек камеры 
		 */
		...
		public function render():void {
			...
			numTriangles = 0;
			if (context3d != null) {
				context3dCachedPrograms = cachedContext3dCachedPrograms[context3d];
				if (context3dCachedPrograms == null) {
					context3dCachedPrograms = new Object();
					cachedContext3dCachedPrograms[context3d] = context3dCachedPrograms;
				}
...
//					projectionMatrixData[13] = 0;
				...
				if (window) {
					projectionMatrixData[0] = focalLength/viewSizeX*window.width/width;
					projectionMatrixData[5] = -focalLength/viewSizeY*window.height/height;
				} else {
				...
				if (root != this && root.visible) {
					...
					if (shadowCaster != null) {
						for (var j:int = shadowCaster.numSplits - 1; j >= 0; j--) {
							...
							shadowCaster.update(this, context3d, j);
		...
		alternativa3d function drawMaterial(material:Material):void {
			planeMesh.geometry.update(context3d);
