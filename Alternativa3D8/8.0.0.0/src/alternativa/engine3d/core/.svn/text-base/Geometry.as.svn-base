...
//				newFace.material = face.material;
		...
		public function calculateTBNMatrices(addAttributes:Boolean = true):Dictionary {
			var vertexTBNs:Dictionary = calculateVertexTBNs();
			for (var key: * in vertexTBNs) {
				var vertex:Vertex = key;
				var vector:Vector.<Vector3D> = vertexTBNs[vertex];
				var averageTangetX:Vector3D = new Vector3D(0, 0, 0, 0);
				
				var len:int = vector.length;
				for (var i:int = 0; i < len; i++) {
				
					var tangentX:Vector3D = vector[i];
					averageTangetX = averageTangetX.add(tangentX);
					averageTangetX.w = tangentX.w;								
				}
				
				averageTangetX.x = averageTangetX.x/len;
				averageTangetX.y = averageTangetX.y/len;
				averageTangetX.z = averageTangetX.z/len;
				if (addAttributes) {
					vertex.attributes = Vector.<Number>([averageTangetX.x, averageTangetX.y, averageTangetX.z, averageTangetX.w]);
				}
			}
			return vertexTBNs;
		}
		...
		private function calculateVertexTBNs():Dictionary {
			...
			for (var key: * in faces) {
				...
				var vertex0:Vertex = vertices[0]; 
				var vertex1:Vertex = vertices[1]; 
				var vertex2:Vertex = vertices[2];
														
				var deltaPos1:Vector3D = new Vector3D(vertex1.x - vertex0.x, vertex1.y - vertex0.y, vertex1.z - vertex0.z);
				var deltaPos2:Vector3D = new Vector3D(vertex2.x - vertex0.x, vertex2.y - vertex0.y, vertex2.z - vertex0.z);
				
				var deltaUV1:Point = new Point(vertex1.u - vertex0.u, vertex1.v - vertex0.v);
				var deltaUV2:Point = new Point(vertex2.u - vertex0.u, vertex2.v - vertex0.v);
			
				var invdet:Number = 1/(deltaUV1.x*deltaUV2.y - deltaUV2.x*deltaUV1.y);
				
				var pqMatrix:Vector.<Vector.<Number>> = new Vector.<Vector.<Number>>();
				pqMatrix[0] = new Vector.<Number>();
				pqMatrix[1] = new Vector.<Number>();
				pqMatrix[0][0] = deltaPos1.x;
				pqMatrix[0][1] = deltaPos1.y;
				pqMatrix[0][2] = deltaPos1.z;
				pqMatrix[1][0] = deltaPos2.x;
				pqMatrix[1][1] = deltaPos2.y;
				pqMatrix[1][2] = deltaPos2.z;
				
				var stMatrix:Vector.<Vector.<Number>> = new Vector.<Vector.<Number>>();
				stMatrix[0] = new Vector.<Number>();
				stMatrix[1] = new Vector.<Number>();
				stMatrix[0][0] =  deltaUV2.y * invdet;
				stMatrix[0][1] = -deltaUV1.y * invdet;
				stMatrix[1][0] = -deltaUV2.x * invdet;
				stMatrix[1][1] =  deltaUV1.x * invdet;
				var tbMatrix:Vector.<Vector.<Number>> = new Vector.<Vector.<Number>>();
				tbMatrix[0] = new Vector.<Number>();
				tbMatrix[1] = new Vector.<Number>();
				// stMatrix * pqMatrix
				...
				var tangentX:Vector3D = new Vector3D(tbMatrix[0][0], tbMatrix[0][1], tbMatrix[0][2], 0);
				var tangentY:Vector3D = new Vector3D(tbMatrix[1][0], tbMatrix[1][1], tbMatrix[1][2], 0);
				

				var t:Vector3D = tangentX.clone();
				
				var normal:Vector3D = new Vector3D(0, 0, 1, 0);
				var dot:Number = normal.dotProduct(t);
				
				tangentX = t.subtract(new Vector3D(normal.x*dot, normal.y*dot, normal.z*dot, 0));
				tangentX.normalize(); 
				
        		dot = normal.crossProduct(t).dotProduct(tangentY);
        		// Calculate handedness
        		tangentX.w = dot < 0 ? -1 : 1;
							
				var vector:Vector.<Vector3D> = vertexTBNs[vertex0];
				if (vector == null) {
					vector = new Vector.<Vector3D>();
				}
				vector.push(tangentX);
				vertexTBNs[vertex0] = vector;
				
				vector = vertexTBNs[vertex1];
				if (vector == null) {
					vector = new Vector.<Vector3D>();
				}
				vector.push(tangentX);
				vertexTBNs[vertex1] = vector;
				
				vector = vertexTBNs[vertex2];
				if (vector == null) {
					vector = new Vector.<Vector3D>();
				}
				vector.push(tangentX);
				vertexTBNs[vertex2] = vector;
			}
