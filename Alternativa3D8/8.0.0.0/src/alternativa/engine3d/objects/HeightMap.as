		...
		public function HeightMap(heightMap:BitmapData) {
			...
			for (var i:int = 0; i < mapWidth; i++) {
				...
				for (var j:int = 0; j < mapHeight;j++) {
					//colorTransform.color = heightMap.getPixel(i, j);
					//vector[j] = (colorTransform.redOffset + colorTransform.greenOffset + colorTransform.blueOffset)*255/765;
//					if (colorTransform.redOffset == 255) {
//						trace("red", colorTransform.redOffset);
//					}
//					vector[j] = colorTransform.redOffset;
					var color:uint = heightMap.getPixel(i, j);
					vector[j] = (color >> 16) & 0xFF;
		...
		public function getHeight(x:uint, y:uint):uint {
		...
		public function generateNormalMap():BitmapData {
			...
			for (var i:int = 0; i < mapWidth - 1; i++) {
				for (var j:int = 0; j < mapHeight - 1; j++) {
					var c:Number = getHeight(i, j)/255;
					var dx:Number = (c - getHeight(i, j+1)/255)*scale;
					var dy:Number = (c - getHeight(i+1, j)/255)*scale;
